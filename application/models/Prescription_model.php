...
class Prescription_model extends MY_Model
{
    ...
    public function vitalsdetails($id)
    {
        $query = "SELECT * FROM visit_details WHERE opd_details_id = $id";
        $query = $this->db->query($query);
        return $query->row();

    }

    public function getcasesheetbyid($id)
    {
        $sql = "SELECT opd_details.*, patients.*, visit_details.*, staff.*, patients.id AS patient_id
                FROM opd_details 

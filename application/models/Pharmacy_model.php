...
class Pharmacy_model extends MY_Model
{
    ...
    {
        $this->datatables
            ->select('pharmacy.*,medicine_category.id as medicine_categoryid,medicine_category.medicine_category,(SELECT sum(available_quantity) FROM `medicine_batch_details` WHERE pharmacy_id=pharmacy.id) as `total_qty`,(SELECT sum(quantity) FROM `medicine_bad_stock` WHERE pharmacy_id=pharmacy.id) as `total_medicine_bad_stock`')
    ...
    {
        $sql = "SELECT medicine_batch_details.*, 
                    IFNULL((SELECT SUM(quantity) FROM pharmacy_bill_detail WHERE medicine_batch_detail_id = medicine_batch_details.id), 0) as used_quantity, 
                    IFNULL((SELECT SUM(quantity) FROM medicine_bad_stock WHERE medicine_batch_details_id = medicine_batch_details.id), 0) as total_medicine_bad_stock 
                FROM medicine_batch_details 
                WHERE medicine_batch_details.id = " . $this->db->escape($medicine_batch_detail_id);
    ...
    public function getStackCount($medicine_batch_id,$pharmacy_id)
    {      
        $this->db->select('SUM(quantity) as total_stack_count');
        $this->db->where('pharmacy_id', $pharmacy_id);
        $query1 = $this->db->get('medicine_batch_details');
        $total_stack_count = $query1->row()->total_stack_count ?? 0;    
     
        $this->db->select('SUM(quantity) as bad_stack_count');
        $this->db->where('medicine_batch_details_id', $medicine_batch_id);
        $this->db->where('pharmacy_id', $pharmacy_id);
        $query2 = $this->db->get('medicine_bad_stock');
        $bad_stack_count = $query2->row()->bad_stack_count ?? 0;
    
        return $total_stack_count - $bad_stack_count;
    }

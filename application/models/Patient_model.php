...
class Patient_model extends MY_Model
{
    ...
    //             FROM transactions
    //             INNER JOIN staff ON staff.name = transactions.received_by_name
    //             WHERE transactions.received_by_name != ''
    ...
    public function searchDataTablePatientRecord($searchterm, $from_date = null, $to_date = null)
    {
        $i = 1;
        $custom_fields = $this->customfield_model->get_custom_fields('patient', 1);
        ...
        $this->datatables
            ...
            ->or_like('patients.address', $searchterm)
            ->or_like('patients.guardian_name', $searchterm)
            ->or_like('patients.ABHA_number', $searchterm)
        ...
        if (!empty($from_date) && !empty($to_date)) {
            $this->datatables->where('DATE(patients.created_at) >=', date('Y-m-d', strtotime($from_date)));
            $this->datatables->where('DATE(patients.created_at) <=', date('Y-m-d', strtotime($to_date)));
        }
    ...
    public function getAllopdRecord()
    {
        ...
        $this->datatables
            ...
            ->orderable('opd_details.id,patients.patient_name,patients.id,patients.guardian_name,patients.gender,patients.mobileno,staff.name' . $custom_field_column)
    ...
    {
        $this->db->select('visit_details.*,appointment.id as appointment_no,appointment_queue.position as appointment_serial_no,transactions.id AS transaction_id,
        department.department_name as department_name,organisation.organisation_name,opd_details.id as opdid,opd_details.case_reference_id,
        opd_details.patient_id,patients.patient_name,patients.id as patient_id,patients.age,patients.month,patients.day,
        patients.dob,patients.guardian_name,patients.gender,patients.marital_status,patients.mobileno,patients.email,patients.address,
        patients.insurance_id,patients.insurance_validity,patients.ABHA_number,patients.known_allergies,patients.image as patient_image,
        blood_bank_products.name as blood_group_name,staff.name,staff.surname,staff.employee_id,patients.id as `patient_id`')->from('visit_details');
        $this->db->join('opd_details', 'opd_details.id = visit_details.opd_details_id');
        $this->db->join('patients', 'patients.id = opd_details.patient_id');
        $this->db->join('blood_bank_products', 'blood_bank_products.id = patients.blood_bank_product_id', 'left');
    ...
    public function getopdvisitDetailsbyvisitid($visitid)
    {
        ...
        $field_variable = implode(',', $field_var_array);
        $this->db->select('visit_details.*,visit_details.known_allergies as visit_known_allergies, transactions.payment_mode,transactions.amount,transactions.cheque_no,transactions.cheque_date,transactions.note as payment_note,transactions.payment_date,organisation.organisation_name,opd_details.id as opdid,opd_details.case_reference_id,opd_details.patient_id,patients.patient_name,patients.id as patient_id,patients.age,patients.month,patients.day,patients.dob,patients.guardian_name,patients.gender,patients.marital_status,patients.mobileno,patients.email,patients.address,patients.insurance_id,patients.insurance_validity,patients.ABHA_number,patients.known_allergies,patients.image as patient_image,blood_bank_products.name as blood_group_name,staff.name,staff.surname,staff.employee_id,patients.id as `patient_id`,' . $field_variable)->from('visit_details');
        $this->db->join('opd_details', 'opd_details.id = visit_details.opd_details_id');
        $this->db->join('patients', 'patients.id = opd_details.patient_id');
        $this->db->join('blood_bank_products', 'blood_bank_products.id = patients.blood_bank_product_id', 'left');
        $this->db->join('staff', 'staff.id = visit_details.cons_doctor');
    ...
    public function getAllopdvisitRecord($patientid)
    {
        ...
        $this->datatables
            ...
            ->join('patient_charges', 'opd_details.id=patient_charges.opd_id', 'left')
            ->join('appointment', 'appointment.visit_details_id = visit_details.id', 'left')
    ...
    public function getAllipdRecord()
    {
        ...
        $custom_field_column = (empty($custom_field_column_array)) ? "" : "," . implode(',', $custom_field_column_array);
        $this->datatables
            ->select('patients.*,bed.name as bed_name,bed_group.name as bedgroup_name, floor.name as floor_name,ipd_details.date,ipd_details.id as ipdid,ipd_details.case_reference_id,ipd_details.credit_limit as ipdcredit_limit,ipd_details.case_type,appointment.id AS appointment_id,staff.name,staff.surname,staff.employee_id' . $field_variable)
            ...
            ->join('appointment', 'appointment.case_reference_id = ipd_details.case_reference_id', "left")
    ...
    public function getDetailsByPatientId($patient_id, $search)
    {
        $patient_details = array();
        $opd_details = array();
        $ipd_details = array();
        ...
        if ($search == "patient_id") {
            $ipd_details = $this->getDetailsipdByPatientId($patient_id);
            $opd_details = $this->getDetailsopdByPatientId($patient_id);
        } else {
            $opd_details = $this->getDetailsopdByPatientId($patient_id);
            $ipd_details = $this->getDetailsipdByPatientId($patient_id);
        }
        ...
        if (!empty($ipd_details) || !empty($opd_details)) {
            $source = !empty($ipd_details) ? $ipd_details : $opd_details;


            $patient_details['patient_id'] = $source['patient_id'];
            $patient_details['patient_name'] = $source['patient_name'];
            $patient_details['dob'] = $source['dob'];
            $patient_details['age'] = $source['age'];
            $patient_details['month'] = $source['month'];
            $patient_details['day'] = $source['day'];
            $patient_details['image'] = $source['image'];
            $patient_details['mobileno'] = $source['mobileno'];
            $patient_details['email'] = $source['email'];
            $patient_details['gender'] = $source['gender'];
            $patient_details['blood_group'] = $source['blood_group'];
            $patient_details['address'] = $source['address'];
            $patient_details['guardian_name'] = $source['guardian_name'];
            $patient_details['is_dead'] = $source['is_dead'];
            $patient_details['insurance_id'] = $source['insurance_id'];
            $patient_details['insurance_validity'] = $source['insurance_validity'];
        }


        if (!empty($ipd_details)) {
            $patient_details['ipd'] = array(
                'ipdid' => $ipd_details['ipdid'],
                'date' => $ipd_details['date'],
                'discharged' => $ipd_details['discharged'],
                'credit_limit' => $ipd_details['credit_limit'],
                'bed_name' => $ipd_details['bed_name'],
                'bed_id' => $ipd_details['bed_id'],
                'bedgroup_name' => $ipd_details['bedgroup_name'],
                'floor_name' => $ipd_details['floor_name']
            );
        } else {
            $patient_details['ipd'] = array(
                'ipdid' => '',
        ...
        if (!empty($opd_details)) {
            $patient_details['opd'] = array(
                'opdid' => $opd_details['opdid'],
                'discharged' => $opd_details['discharged'],
                'appointment_date' => $opd_details['appointment_date']
            );
        } else {
            $patient_details['opd'] = array(
                'opdid' => '',
    ...
    public function getDetailsipdByPatientId($patient_id)
    {
        return $this->db->select('patients.id as patient_id,patients.patient_name,patients.dob,patients.age,patients.month,patients.day,patients.image,patients.mobileno,patients.email,patients.gender,patients.blood_group,patients.address,patients.guardian_name,patients.is_dead,patients.insurance_id,patients.insurance_validity,ipd_details.id as ipdid,`ipd_details`.`date`,ipd_details.discharged,ipd_details.credit_limit,ipd_details.date,bed.name as bed_name,bed.id as bed_id,bed_group.name as bedgroup_name,floor.name as floor_name')
            ->from('patients')
    ...
    public function getDetailsByIPDId($ipd_id)
    {
        $patient_details = array();
        $ipd_details = $this->getDetailsByIPD($ipd_id);
    ...
    public function getDetailsByIPD($ipd_id)
    {
        return $this->db->select('patients.id as patient_id, patients.patient_name, patients.dob, patients.age, patients.month, patients.day, patients.image, patients.mobileno, patients.email, patients.gender, patients.blood_group, patients.address, patients.guardian_name, patients.is_dead, patients.insurance_id, patients.insurance_validity, ipd_details.id as ipdid, ipd_details.date, ipd_details.discharged, ipd_details.credit_limit, bed.name as bed_name, bed.id as bed_id, bed_group.name as bedgroup_name, floor.name as floor_name')
            ->from('patients')
            ->join('ipd_details', 'ipd_details.patient_id = patients.id', 'inner')
            ->join('bed', 'ipd_details.bed = bed.id', 'left')
            ->join('bed_group', 'ipd_details.bed_group_id = bed_group.id', 'left')
            ->join('floor', 'floor.id = bed_group.floor', 'left')
            ->where('ipd_details.id', $ipd_id)
            ->get()
            ->row_array();
    }

    public function getDetailsByOPDId($opd_id)
    {
        $patient_details = array();
        $opd_details = $this->getDetailsopdByOPDId($opd_id);
        $ipd_details = array();
    ...
    public function getDetailsopdByOPDId($opd_id)
    {
        return $this->db->select('patients.id as patient_id, patients.patient_name, patients.dob, patients.age, patients.month, patients.day, patients.image, patients.mobileno, patients.email, patients.gender, patients.blood_group, patients.address, patients.guardian_name, patients.is_dead, patients.insurance_id, patients.insurance_validity, opd_details.id as opdid, opd_details.discharged, visit_details.appointment_date')
            ->from('patients')
            ->join('opd_details', 'opd_details.patient_id=patients.id', 'inner')
    ...
    public function getpatientoverview($patient_id)
    {
        ...
        $patient_details['patient']['visitdetails'] = $this->db
            ->select('opd_details.case_reference_id,opd_details.id as opd_id,opd_details.patient_id as patientid,opd_details.is_ipd_moved,max(visit_details.id) as visit_id,visit_details.appointment_date,appointment.id AS appointment_id,visit_details.refference,visit_details.symptoms,patients.id as pid,patients.patient_name,staff.id as staff_id,staff.name,staff.surname,staff.employee_id,consult_charges.standard_charge,patient_charges.apply_charge')
            ->join('visit_details', 'opd_details.id = visit_details.opd_details_id', "left")->join('staff', 'staff.id = visit_details.cons_doctor', "inner")->join('patients', 'patients.id = opd_details.patient_id', "inner")->join('consult_charges', 'consult_charges.doctor=visit_details.cons_doctor', 'left')
            ->join('patient_charges', 'opd_details.id=patient_charges.opd_id', 'left')->order_by('visit_details.id', 'desc')
            ->where('opd_details.patient_id', $patient_id)
            ->join('appointment', 'visit_details.id = appointment.visit_details_id')
    ...
    public function getprescription_detail($opdid)
    {
        if (empty($opdid)) {
            return [];
        }
        $this->db->select('*')->from('opd_prescription')->where('opd_id', $opdid);
        $query = $this->db->get();
        return $query->result_array();
    }
}
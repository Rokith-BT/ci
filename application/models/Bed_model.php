...
class Bed_model extends MY_Model
{
    ...
    public function bed_active()
    {
        $result = $this->db->select("
                ...
                bed.name, 
                bed.is_active,
                GROUP_CONCAT(DISTINCT patients.id) as pid,
                GROUP_CONCAT(DISTINCT patients.is_active) as patient_status,
                GROUP_CONCAT(DISTINCT patients.patient_name) as patient_name,
                GROUP_CONCAT(DISTINCT patients.gender) as gender,
                GROUP_CONCAT(DISTINCT patients.guardian_name) as guardian_name,
                GROUP_CONCAT(DISTINCT patients.mobileno) as mobileno,
                GROUP_CONCAT(DISTINCT ipd_details.bed) as bid,
                GROUP_CONCAT(DISTINCT ipd_details.discharged) as ipd_discharged
            ")

...
class Transaction_model extends MY_Model
{
    ...
    public function ipdopdPaymentByCaseId($case_id, $search)
    {
        $this->db->select('SUM(amount) as total_pay')
            ->where("transactions.case_reference_id", $case_id)
        ...
        if ($search == 'opd_id') {
            $this->db->group_start()
                ->where("transactions.section", "Appointment")
                ->or_where("transactions.section", "OPD")
                ->group_end();
        }
    ...
    public function getPatientPaymentsByCaseId($case_id, $module_type, $id, $search, $patient_id)
    {


        $this->db->select('transactions.*')->order_by("transactions.id", "desc");
        if ($search == "patient_id") {
            $this->db->where("transactions.patient_id", $patient_id);
            $this->db->where("transactions.section", $module_type);
        } else if ($search == "opd_id") {
            $this->db->where("transactions.case_reference_id", $case_id);
        } else if ($search == "case_id") {
            $this->db->where("transactions.case_reference_id", $patient_id);
            $this->db->where("transactions.section", $module_type);
        } else {
            $this->db->where("transactions.ipd_id", $patient_id);
            $this->db->where("transactions.section", $module_type);
        }
    ...
    public function getTransactionByCaseId($case_id, $search)
    {
        if ($search == "patient_id") {
            $sql = "SELECT * FROM `transactions` WHERE patient_id =" . $this->db->escape_str($case_id) . " and type='payment'";
        } else if ($search == "opd_id") {
            $sql = "SELECT * FROM `transactions` WHERE case_reference_id =" . $this->db->escape_str($case_id) . " and type='payment'";
        } else if ($search == "ipd_id") {
            $sql = "SELECT * FROM `transactions` WHERE ipd_id =" . $this->db->escape_str($case_id) . " and type='payment'";
        } else {
            $sql = "SELECT * FROM `transactions` WHERE case_reference_id =" . $this->db->escape_str($case_id) . " and type='payment'";
        }
    ...
    public function get_ipd_charges_by_caseId($case_id, $search)
    {
        $this->db->select('patient_charges.*, charge_categories.name as charge_category_name, charges.charge_category_id, charges.standard_charge, 
            charges.name as charge_name, charge_units.unit, charge_type_master.id as charge_type_master_id, 
            ipd_details.patient_id as ipd_patient_id, ipd_patient.patient_name as ipd_patient_name, 
            ipd_details.case_reference_id as ipd_case_reference_id, 
            tax_category.name as apply_tax, tax_category.percentage')
            ->join('ipd_details', 'patient_charges.ipd_id = ipd_details.id', 'inner')
            ->join('patients as ipd_patient', 'ipd_details.patient_id = ipd_patient.id', 'inner')
            ...
            ->join('charge_type_master', 'charge_categories.charge_type_id = charge_type_master.id', 'inner')
            ->join('charge_units', 'charges.charge_unit_id = charge_units.id', 'left');

        if ($search == "patient_id") {
            $this->db->where('ipd_details.patient_id', $case_id);
        } else if ($search == "case_id") {
            $this->db->where('ipd_details.case_reference_id', $case_id);
        } else if ($search == "ipd_id") {
            $this->db->where('patient_charges.ipd_id', 10);
        }

        return $this->db->get('patient_charges')->result_array();
    }


    public function get_ipdopdchargebycaseId($case_id, $search, $module_type = null, $id = null, $patient_id = null)
    {
        $this->db->select('patient_charges.*, charge_categories.name as charge_category_name, charges.charge_category_id, charges.standard_charge, 
            charges.name as charge_name, charge_units.unit, charge_type_master.id as charge_type_master_id, 
            ipd_details.patient_id as ipd_patient_id, ipd_patient.patient_name as ipd_patient_name, 
            opd_details.patient_id as opd_patient_id, opd_patient.patient_name as opd_patient_name, 
            opd_details.case_reference_id as opd_case_reference_id, ipd_details.case_reference_id as ipd_case_reference_id, 
            tax_category.name as apply_tax, tax_category.percentage')
            ...
            ->join('charge_units', 'charges.charge_unit_id = charge_units.id', 'left');
        if ($search == "patient_id") {
            $this->db->where('opd_details.patient_id', $case_id);
        } else if ($search == "opd_id") {
            $this->db->where('opd_details.case_reference_id', $case_id);
        } else if ($search == "ipd_id") {
            if ($module_type == "ipd_opd") {
                $this->db->where('ipd_details.case_reference_id', $case_id);
            } else {
                $this->db->where('patient_charges.ipd_id', $id);
            }
        } else {
            $this->db->where('ipd_details.case_reference_id', $case_id);
            $this->db->or_where('opd_details.case_reference_id', $case_id);
        }
        return $this->db->get('patient_charges')->result_array();
    }
    ...
    public function getDateRangeByType($search_type, $date_from = null, $date_to = null)
    {
        $now = new DateTime();
        $today = clone $now;
        switch ($search_type) {
            case 'today':
                $fromDate = $today->setTime(0, 0, 0)->format('Y-m-d H:i:s');
                $toDate = $today->setTime(23, 59, 59)->format('Y-m-d H:i:s');
                break;
            case 'this_week':
                $fromDate = (clone $today)->modify('monday this week')->setTime(0, 0, 0)->format('Y-m-d H:i:s');
                $toDate = (clone $today)->modify('sunday this week')->setTime(23, 59, 59)->format('Y-m-d H:i:s');
                break;
            case 'last_week':
                $fromDate = (clone $today)->modify('monday last week')->setTime(0, 0, 0)->format('Y-m-d H:i:s');
                $toDate = (clone $today)->modify('sunday last week')->setTime(23, 59, 59)->format('Y-m-d H:i:s');
                break;
            case 'this_month':
                $fromDate = (clone $today)->modify('first day of this month')->setTime(0, 0, 0)->format('Y-m-d H:i:s');
                $toDate = (clone $today)->modify('last day of this month')->setTime(23, 59, 59)->format('Y-m-d H:i:s');
                break;
            case 'last_month':
                $fromDate = (clone $today)->modify('first day of last month')->setTime(0, 0, 0)->format('Y-m-d H:i:s');
                $toDate = (clone $today)->modify('last day of last month')->setTime(23, 59, 59)->format('Y-m-d H:i:s');
                break;
            case 'last_3_month':
                $fromDate = (clone $today)->modify('first day of -3 month')->setTime(0, 0, 0)->format('Y-m-d H:i:s');
                $toDate = (clone $today)->modify('last day of last month')->setTime(23, 59, 59)->format('Y-m-d H:i:s');
                break;
            case 'last_6_month':
                $fromDate = (clone $today)->modify('first day of -6 month')->setTime(0, 0, 0)->format('Y-m-d H:i:s');
                $toDate = (clone $today)->modify('last day of last month')->setTime(23, 59, 59)->format('Y-m-d H:i:s');
                break;
            case 'last_12_month':
                $fromDate = (clone $today)->modify('first day of -12 month')->setTime(0, 0, 0)->format('Y-m-d H:i:s');
                $toDate = (clone $today)->modify('last day of last month')->setTime(23, 59, 59)->format('Y-m-d H:i:s');
                break;
            case 'this_year':
                $year = $today->format('Y');
            ...
            case 'last_year':
                ...
                break;
            case 'period':
                $fromDate = (new DateTime($date_from))->format('Y-m-d H:i:s');
                $toDate = (new DateTime($date_to))->format('Y-m-d H:i:s');
                break;
            default:
                $fromDate = $today->setTime(0, 0, 0)->format('Y-m-d H:i:s');
                $toDate = $today->setTime(23, 59, 59)->format('Y-m-d H:i:s');
    ...
    public function getpaymentmethod_detials($data)
    {
        $payment_summary = [
            'total' => 0,
            'cash' => 0,
        ...
        foreach ($data as $item) {
            ...
            $payment_summary['total'] += $amount;
            $mode = strtolower(trim($item['payment_mode']));
            switch ($mode) {
                case 'cash':
                    $payment_summary['cash'] += $amount;
                    break;
                case 'online':
                    $payment_summary['online'] += $amount;
                    break;
                case 'upi':
                ...
                case 'card':
                    ...
                    break;
                case 'neft':
                    $payment_summary['neft'] += $amount;
                    break;
                case 'cheque':
                    $payment_summary['cheque'] += $amount;
                    break;
                case 'scheme':
                    $payment_summary['scheme'] += $amount;
                    break;
            }
        }

...
class Appoint_priority_model extends MY_Model
{
    ...
    public function Appointment_status($id = null)
    {
        $this->db->select()->from('appointment_status');
        if ($id != null) {
            $this->db->where('appointment_status.id', $id);
        } else {
            $this->db->order_by('appointment_status.id');
        }
        $query = $this->db->get();
        if ($id != null) {
            return $query->row_array();

...
class Staff_model extends MY_Model
{
    ...
    {
        $query = $this->db->query("SELECT t2.id, t2.specialist_name 
        FROM staff AS t1 
        LEFT JOIN specialist AS t2 
        ON JSON_CONTAINS(t1.specialist, CAST(t2.id AS JSON), '$')
        WHERE t1.id = $id");
        return $query->result();
    }
    ...
    {
        $this->db->select('staff.*,blood_bank_products.name AS Blood_group_name,staff_designation.designation as designation,staff_roles.role_id, department.department_name as department,roles.name as user_type,specialist.specialist_name');
        $this->db->join("staff_designation", "staff_designation.id = staff.staff_designation_id", "left");
        $this->db->join("department", "department.id = staff.department_id", "left");
        $this->db->join("staff_roles", "staff_roles.staff_id = staff.id", "left");
        $this->db->join("roles", "staff_roles.role_id = roles.id", "left");
        $this->db->join("blood_bank_products", "staff.blood_group = blood_bank_products.id", "left");
    ...
    public function getStaffbyrole($id, $type = null)
    {
        if ($this->session->has_userdata('hospitaladmin')) {
            $doctor_restriction = $this->session->userdata['hospitaladmin']['doctor_restriction'];
            $userdata = $this->customlib->getUserData();
            $role_id = $userdata['role_id'];
            if ($doctor_restriction == 'enabled' && $role_id == $id) {
                $user_id = $userdata["id"];
        ...
        if ($type == "opd") {
            ...
            doctor_shift.day
        ');
            $this->db->join("doctor_shift", "doctor_shift.staff_id = staff.id", "inner");
            $this->db->where("doctor_shift.start_time <=", $currenttime);
            $this->db->where("doctor_shift.end_time >=", $currenttime);
            $this->db->where("doctor_shift.day", $currentDay);
        } else {
            $this->db->select('
            staff.id,
            staff.name,
    ...
    public function opdstaff($id)
    {
        ...
        $this->db->group_by(['staff.id', 'doctor_shift.id']);
        $this->db->order_by("staff.name", 'asc');
    ...
    public function getcountofappointment($id)
    {
        $sql = "SELECT COUNT(*) AS Future_appointment_count FROM appointment WHERE date >= CURDATE() AND doctor = ?;";
        $query = $this->db->query($sql, array($id));
        return $query->row()->Future_appointment_count;
    }

    public function get_leaves($month, $year, $id)
    {
        $month_number = date('m', strtotime($month));
        $get_lop_count = "SELECT id FROM leave_types WHERE type = 'lop'";
        $lop_count = $this->db->query($get_lop_count)->row()->id ?? 0;

        $sql = "SELECT SUM(leave_days) AS total_leave_days 
                FROM staff_leave_request 
                WHERE (MONTH(leave_from) = ? OR MONTH(leave_to) = ?) 
                AND YEAR(leave_from) = ? 
                AND staff_id = ? 
                AND leave_type_id != ?";

        $query = $this->db->query($sql, array($month_number, $month_number, $year, $id, $lop_count));
        return $query->row() ? $query->row()->total_leave_days : 0;
    }

    public function staff_attendance($month, $year, $id)
    {
        $month_number = date('m', strtotime($month));
        $sql = "SELECT COUNT(*) AS absent FROM staff_attendance WHERE MONTH(`date`) = ? AND YEAR(`date`) = ? AND staff_id = ? AND staff_attendance_type_id = ?";
        $query = $this->db->query($sql, array($month_number, $year, $id, 3));
        return $query->row() ? $query->row()->absent : 0;
    }
}

...
class Charge_model extends MY_Model
{
    ...
    {
        $sql   = "SELECT organisations_charges.id,organisations_charges.org_charge,
        organisations_charges.org_charge,organisation.organisation_name,
        organisation.id as org_id FROM organisations_charges RIGHT OUTER JOIN 
        organisation ON organisations_charges.org_id = organisation.id AND 
        organisations_charges.charge_id = " . $charge_id . " ORDER BY organisation.id";
    ...
    {
        $query = $this->db->query("SELECT tpa_doctorcharges.id,tpa_doctorcharges.org_charge,
        organisation.organisation_name,organisation.id as org_id FROM tpa_doctorcharges 
        RIGHT OUTER JOIN organisation ON tpa_doctorcharges.org_id = organisation.id AND 
        tpa_doctorcharges.charge_id = '$charge_id' ORDER BY organisation.id");
        return $query->result_array();
    }
    ...
    public function getChargeByAll($id)
    {
        $query = $this->db->select('patient_charges.*, charge_categories.name as charge_category_name,
            charges.charge_category_id, charges.standard_charge, charges.name as charge_name,
            charge_units.unit, charge_type_master.id as charge_type_master_id, ipd_details.patient_id as ipd_patient_id,
            ipd_patient.patient_name as ipd_patient_name, opd_details.patient_id as opd_patient_id,
            opd_patient.patient_name as opd_patient_name, opd_details.case_reference_id as opd_case_reference_id,
            ipd_details.case_reference_id as ipd_case_reference_id, tax_category.name as apply_tax, tax_category.percentage')
            ->join('opd_details', 'patient_charges.opd_id = opd_details.id', 'left')
            ->join('patients as opd_patient', 'opd_details.patient_id = opd_patient.id', 'left')
            ->join('ipd_details', 'patient_charges.ipd_id = ipd_details.id', 'left')
            ->join('patients as ipd_patient', 'ipd_details.patient_id = ipd_patient.id', 'left')
            ...
            ->join('charge_units', 'charges.charge_unit_id = charge_units.id', 'left')
            ->where('patient_charges.opd_id', $id)
            ->get('patient_charges');
    
        return $query->result_array(); // Fetch all data as an array
    }
    ...
    public function getopdChargesbyCaseId($case_id, $search)
    {
        ini_set('display_errors', 1);
        ini_set('display_startup_errors', 1);
        error_reporting(E_ALL);    
        $query = $this->db->select('patient_charges.*, charge_categories.name as charge_category_name, charge_type_master.charge_type, charges.charge_category_id, charges.standard_charge, organisations_charges.id as oid, organisations_charges.org_charge, charge_units.unit, charges.name, opd_details.id as opd_id, opd_details.case_reference_id, paideamount.total_paid')
            ->join('opd_details', 'patient_charges.opd_id = opd_details.id')
        ...
        if ($search !== NULL && $search == "patient_id") {
            $this->db->join('(SELECT SUM(amount) as total_paid FROM transactions WHERE patient_id = ' . $this->db->escape($case_id) . ' AND section = "OPD") as paideamount', '1=1', 'left');
            $this->db->where('patient_charges.patient_id', $case_id);
            $this->db->where('patient_charges.opd_id IS NOT NULL');
        } else if ($search !== NULL && $search == "opd_id") {
            $this->db->join('(SELECT SUM(amount) as total_paid FROM transactions WHERE case_reference_id = ' . $this->db->escape($case_id) . ' AND (section = "OPD" OR section = "Appointment")) as paideamount', '1=1', 'left');
            $this->db->where('opd_details.id', $case_id);
        } else if ($search !== NULL && $search === "ipd_id") {
            return null;
        } else {
            $this->db->join('(SELECT SUM(amount) as total_paid FROM transactions WHERE case_reference_id = ' . $this->db->escape($case_id) . ' AND section = "OPD") as paideamount', '1=1', 'left');
            $this->db->where('opd_details.case_reference_id', $case_id);
        }
        $this->db->group_by('patient_charges.id');
        return $this->db->get('patient_charges')->result_array();
    }
    
    public function getipdChargesbyCaseId($case_id, $search)
    {
                ini_set('display_errors', 1);
        ini_set('display_startup_errors', 1);
        error_reporting(E_ALL);
        $query = $this->db->select('patient_charges.*, ipd_details.patient_id, charge_categories.name as charge_category_name, charge_type_master.charge_type, charges.charge_category_id, charges.standard_charge, organisations_charges.id as oid, organisations_charges.org_charge, charge_units.unit, charges.name, ipd_details.id as ipd_id, ipd_details.case_reference_id, paideamount.total_paid')
            ->join('ipd_details', 'patient_charges.ipd_id = ipd_details.id')
            ->join('patients', 'ipd_details.patient_id = patients.id')
            ->join('charges', 'patient_charges.charge_id = charges.id', 'inner')
            ->join('charge_categories', 'charge_categories.id = charges.charge_category_id', 'inner')
        ...
        if ($search !== NULL && $search === "patient_id") {
            $query->join('(SELECT SUM(amount) as total_paid, patient_id FROM transactions WHERE section = "IPD" GROUP BY patient_id) as paideamount', 'paideamount.patient_id = ipd_details.patient_id', 'left');
            $this->db->where('ipd_details.patient_id', $case_id);
        }else if($search !== NULL && $search === "ipd_id"){
            $query->join('(SELECT SUM(amount) as total_paid FROM transactions WHERE ipd_id = ' . $this->db->escape($case_id) . ' AND section = "IPD") as paideamount', '1=1', 'left');
            $this->db->where('ipd_id', $case_id);
        } else {
            $query->join('(SELECT SUM(amount) as total_paid FROM transactions WHERE case_reference_id = ' . $this->db->escape($case_id) . ' AND section = "IPD") as paideamount', '1=1', 'left');
            $this->db->where('ipd_details.case_reference_id', $case_id);
        }
    
        $query = $this->db->get('patient_charges');
    ...
    public function getAppointmentCharges($id)
    {
        $this->db->select("patient_charges.opd_id, visit_details.patient_charge_id")
                 ->from("patient_charges")
                 ->join("visit_details", "visit_details.opd_details_id = patient_charges.opd_id", "LEFT")
                 ->where("patient_charges.id", $id);
        
        $result = $this->db->get()->row();
    
        return $result ? $result->patient_charge_id : null;
    }

...
class Customlib
{
    ...
    {
        return $this->CI->Setting_model->get();
    }
    ...
    public function getSessionPrefixByType($type = "")
    {
        ...
        if (array_key_exists($type, $prefix_array)) {
    ...
    public function getPrefixnameByType($type = "")
    {
        ...
        return isset($prefix_name[$type]) ? $prefix_name[$type] : null;
    }
    ...
    public function YYYYMMDDTodateFormat($date = null)
    {
        ...
        if (trim((string)$date) == "" || substr((string)$date, 0, 10) == '0000-00-00') {
            return "";
        }
    ...
    {
        $CI = &get_instance();
        $sessionData = $CI->session->userdata('hospitaladmin');
        $api_base_url =  $CI->config->item('api_base_url_casesheet');
        $email = $sessionData["email"];
        $api_url = $api_base_url . "login/getAccessToken?username=" . $email;
        ...
        log_message('debug', 'Token Refresh Response: ' . $response);
        return $response;
    }

    public function getpagenation($api_url)
    {
        ...
        $offset = ($page - 1) * $limit;
        $CI = &get_instance();
        $api_base_url = $CI->config->item('api_base_url');
        $hospitaldata = $CI->session->userdata('hospitaladmin');
        $token = $hospitaldata['accessToken'] ?? '';
        if ($search_value != "") {
            $url = "{$api_base_url}{$api_url}?page={$page}&limit={$limit}&search={$search_value}";
        } else {
            $url = "{$api_base_url}{$api_url}?page={$page}&limit={$limit}";
        }
        ...
        if (!empty($responseData['status_code']) && $responseData['status_code'] == 403) {
            $newToken = $this->refreshToken();
            $hospitaldata['accessToken'] = $newToken;
            $CI->session->set_userdata('hospitaladmin', $hospitaldata);
    ...
    public function getpagenationfull($api_url)
    {
        $search_value = isset($_GET['search']) ? $_GET['search'] : "";
        $draw = isset($_GET['draw']) ? intval($_GET['draw']) : 1;
        $limit = isset($_GET['limit']) ? intval($_GET['limit']) : 10;

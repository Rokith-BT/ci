                                        ...
                                        <div class="cheque_div" style="display:none;">
                                            ...
                                            <div class="col-sm-4">
                                                <div class="form-group">
                                                    ...
                                                    <input type="file" class="filestyle form-control"  id="document_report" name="document">
                                    ...
                                    <div class="row">
                                        ...
                                        <div class="cheque_div" style="display:none;">
                                            ...
                                            <div class="col-sm-4">
                                                <div class="form-group">
                                                    <label><?php echo $this->lang->line('attach_document'); ?></label>
                                                    <input type="file" class="filestyle form-control"   name="document" id="document1">
                                                    <span class="text-danger"><?php echo form_error('document'); ?></span>
                                                </div>
                                            </div>
                                        </div>
...
$(document).ready(function () {
    $("#formcall").on('submit', function (e) {
        ...
        if (!formData.get("vehicle_no")) {
            errorMsg("Please select a Vehicle Model.");
            return false;
        }
        ...
        if (!formData.get("charge_category_id")) {
            errorMsg("Please select a Charge Category.");
            return false;
        }
        if(!formData.get("code"))
        {
            errorMsg("Please select a Charge.");
            return false;
        }

        formObject["patient_id"] = formData.get("patient_id");
        formObject["case_reference_id"] = formData.get("case_reference_id");
        formObject["vehicle_id"] = formData.get("vehicle_no");
        formObject["driver"] = formData.get("driver");
        formObject["date"] = convertToDateTime(formData.get("date"));
        formObject["call_from"] = formData.get("call_from") || "";
        formObject["call_to"] = formData.get("call_to") || "";
        formObject["charge_category_id"] = formData.get("charge_category_id");
        formObject["charge_id"] = formData.get("code");
        formObject["standard_charge"] = formData.get("standard_charge");
        formObject["tax_percentage"] = formData.get("tax_percentage");
        formObject["amount"] = formData.get("payment_amount");
        formObject["net_amount"] = formData.get("net_amount");
        formObject["note"] = formData.get("note") || "";
        formObject["generated_by"] = <?=$data['id']?>;
        formObject["payment_mode"] = formData.get("payment_mode");

        if (formObject["payment_mode"] === 'Cheque') {
            formObject["cheque_no"] = formData.get("cheque_no") || "";
            formObject["cheque_date"] = convertToDateTime(formData.get("cheque_date"));
            formObject["attachment"] = "";
            formObject["attachment_name"] = "";
        }

        formObject["payment_date"] = convertToDateTime(formData.get("payment_date") || formData.get("date"));
        formObject["received_by"] = <?=$data['id']?>;
        formObject["hospital_id"] = <?=$data['hospital_id']?>;

        let file = document.getElementById('document_report').files[0];
        if (file) {
            ...
            $.ajax({
                ...
                success: function (response) {
                    if (response && response.data) {
                        formObject.attachment = response.data;
                        formObject.attachment_name = file.name;
                    }
                    submitPayment(formObject);
                },
                error: function () {
...
function submitPayment(formObject) {
    $.ajax({
        url: '<?=$api_base_url?>ambulance-call',
        type: "POST",
        ...
        success: function (data) {
            if (data?.[0]?.data?.status === "fail") {
                ...
                successMsg(data?.[0]?.data?.message || "Operation successful.");
                $('#myModal').modal('hide');
                if ($("button[type=submit][clicked=true]").attr('name') === "save_print") {
                    printData(data?.id);
                }
            }
        },
        ...
        complete: function () {
            $("button[type=submit][clicked=true]").button('reset');
        }
    });
}
    ...
    function holdModal(modalId) {
        ...
        function delete_bill(id) {
            if (confirm('<?php echo $this->lang->line('delete_confirm'); ?>')) {
                $.ajax({
                    url: '<?=$api_base_url?>ambulance-call/' + id + '?hospital_id=<?=$data['hospital_id']?>',
                    type: 'DELETE',
                    success: function (res) {
...
<script type="text/javascript">
     function deletePayment(id) {
        if (confirm(<?php echo "'" . $this->lang->line('delete_confirm') . "'"; ?>)) {
            $.ajax({
                url: '<?=$api_base_url?>ambulance-call/deleteAmbulancePayment/'+ id + '?Hospital_id=<?=$data['hospital_id']?>',
                type: "DELETE",
                success: function (res) {
                    successMsg(<?php echo "'" . $this->lang->line('delete_message') . "'"; ?>);
                    window.location.reload(true);
...
<script type="text/javascript">
    ...
    $(document).ready(function () {
        $("#formedit").on('submit', function (e) {
            ...
            jsonObject.patient_id = formData.get("patient_id") || "";
            jsonObject.case_reference_id = $('#case_reference_id_edit').val();
            jsonObject.vehicle_id = $('#vehicleno').val();
            jsonObject.driver = formData.get("driver_name") || "";          

            const dateInput = formData.get("date") || "";
            if (dateInput) {
                const date = new Date(dateInput);
                if (!isNaN(date)) {
                    jsonObject.date = date.toISOString().slice(0, 19).replace('T', ' ');
                } else {
                    jsonObject.date = ""; 
                }
            }

            jsonObject.call_from = formData.get("call_from") || "";
            jsonObject.call_to = formData.get("call_to") || "";
            jsonObject.charge_category_id = formData.get("charge_category_id") || "139";
            jsonObject.charge_id = $('#code_edit').val();
            jsonObject.standard_charge = formData.get("standard_charge") || "";
            jsonObject.tax_percentage = formData.get("tax_percentage") || "";
            jsonObject.amount = $('#code_edit').val();
            jsonObject.net_amount = formData.get("net_amount") || "";
            jsonObject.note = formData.get("note") || "";
            jsonObject.generated_by = <?=$data['hospital_id']?>;
            jsonObject.payment_mode = formData.get("payment_mode") || "";           

            if (jsonObject.payment_mode === 'Cheque') {
                jsonObject.cheque_no = formData.get("cheque_no") || "";
                const chequeDateInput = formData.get("cheque_date") || "";
                if (chequeDateInput) {
                    const chequeDate = new Date(chequeDateInput);
                    if (!isNaN(chequeDate)) {
                        jsonObject.cheque_date = chequeDate.toISOString().slice(0, 19).replace('T', ' ');
                    } else {
                        jsonObject.cheque_date = "";
                    }
                }
            ...
            const paymentDateInput = formData.get("payment_date") || formData.get("date");
            if (paymentDateInput) {
                const paymentDate = new Date(paymentDateInput);
                if (!isNaN(paymentDate)) {
                    jsonObject.payment_date = paymentDate.toISOString().slice(0, 19).replace('T', ' ');
                } else {
                    jsonObject.payment_date = ""; 
                }
            }

            jsonObject.received_by = <?=$data['id']?>; 
            jsonObject.hospital_id = <?=$data['hospital_id']?>;

            let file = document.getElementById('document1').files[0];
            if (file) {
                let uploadFormData = new FormData();
                uploadFormData.append('file', file);
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        ...
                        jsonObject.attachment_name = file.name;
                        submitPayment(jsonObject);
                    },
                    error: function() {
            ...
            function submitPayment(jsonObject) {
                // console.log(JSON.stringify(jsonObject,null,2));
                let editid = $('#ambulance_call_edit_id').val();
                $.ajax({
                    url: '<?=$api_base_url?>ambulance-call/'+editid,
                    type: "PATCH",

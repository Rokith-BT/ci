                                            ...
                                            <tr> 
                                                ...
                                                <td class="mailbox-date pull-right noExport">
                                                    ...
                                                    <?php } if ($this->rbac->hasPrivilege('supplier', 'can_delete')) { ?>
                                                        <a  class="btn btn-default btn-xs"  data-toggle="tooltip" title="<?php echo $this->lang->line('delete'); ?>" onclick="delete_iteamsupplier('<?php echo $supplier['id'] ?>')">
...
<div class="modal fade" id="myModal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
    <div class="modal-dialog modal-mid" role="document">
        <div class="modal-content modal-media-content">
            ...
            <form id="itemsupplierlistadd" action="<?php echo site_url('admin/itemsupplier/add') ?>" name="employeeform" method="post" accept-charset="utf-8"  enctype="multipart/form-data">
...
$(document).ready(function() {
    $('#itemsupplierlistadd').on('submit', function(e) {
        e.preventDefault();        
        var formData = {
            item_supplier: $('#name').val(),
            phone: $('#phone').val(),
            email: $('#email').val(),
        ...
        $.ajax({
            url: '<?=$api_base_url?>setup-inventory-item-supplier',
            type: 'POST',
            data: formData,            
            beforeSend: function() {
            ...
            success: function(response) {
                successMsg('Iteam Supplier add successfully'); 
                location.reload();
...
<script>
    ...
    $("#editformadd").on('submit', function (e) {
        ...
        var formData = {
            item_supplier: $('#name1').val(),
            phone: $('#phone1').val(),
            email: $('#email1').val(),
            contact_person_name: $('#contact_person_name1').val(),
            address: $('#address1').val(),
            contact_person_phone: $('#contact_person_phone1').val(),
            contact_person_email: $('#contact_person_email1').val(),
            description: $('#description1').val(),
            Hospital_id:<?=$data['hospital_id']?>             
        }
        let supp_id =  $('#supp_id').val();
        $.ajax({
            url: '<?=$api_base_url?>setup-inventory-item-supplier/' + supp_id,
            type: "PATCH",
            data: formData,
            dataType: 'json',
...
<script>
    function delete_iteamsupplier(id)
    {
        var conf = confirm("Are you sure to delete this record?");
        if (conf == true) {
            $.ajax({
                url: "<?=$api_base_url?>setup-inventory-item-supplier/" + id,
                type: "DELETE",
                dataType: 'json',
                success: function (data) {

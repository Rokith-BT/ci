...
<div class="modal fade" id="addReportModal" role="dialog" aria-labelledby="myModalLabel">
    <div class="modal-dialog modal-lg" role="document">
        <div class="modal-content modal-media-content">
            <form  method="POST" id="form-report_param">
            <div class="modal-header modal-media-header">
                <button type="button" class="close" data-toggle="tooltip" title="<?php echo $this->lang->line('close'); ?>" data-dismiss="modal">&times;</button>
                <div class="modalicon"> 
                    <div id='collection_modal_header'>
...
<script type="text/javascript">
    ...
    $(function () {
        ...
        $('.select2').select2()
            ...
            $this.button('loading');
            $.ajax({
                url: '<?=$api_base_url?>pathology-generate-bill/'+recordId + '?hospital_id=<?=$data['hospital_id']?>',
                type: "DELETE",              
                dataType: 'json',
                 beforeSend: function() {
                    $this.button('loading');
    ...
    $(document).on('submit', '#bill', function(e) {
        ...
        let ipd_prescription_basic_id = parseInt(prescriptionNo.match(/\d+/));
        let jsonData = {
            case_reference_id: parseInt(formData.get('case_reference_id')),
            ipd_prescription_basic_id: ipd_prescription_basic_id,
            patient_id: parseInt(formData.get('patientid')),
            doctor_id: parseInt(formData.get('consultant_doctor')),
            doctor_name: formData.get('doctor_name'),
            total: formData.get('total'),
            discount_percentage: formData.get('discount_percent'),
            discount: formData.get('discount'),
            tax_percentage: formData.get('tax_percent'),
        ...
        totalRows.forEach(function(row, index) {
            pathologyItems.push({
                pathology_bill_id: 1,
                pathology_id: parseInt(formData.get('test_name_' + (index + 1))),
                patient_id: parseInt(formData.get('patientid')),
                reporting_date: new Date(formData.get('reportdate_' + (index + 1))).toISOString().slice(0, 10) + " 11:11:11",
                tax_percentage: formData.get('taxpercent_' + (index + 1)),
                apply_charge: formData.get('amount_' + (index + 1)),
                hospital_id: <?=$data['hospital_id']?>
            });
        });
        ...
        let finalData = {
            ...jsonData,
            pathology_items: pathologyItems
        };

        let editid = $('#pathology_billing_id').val();
        let api1, method, api2;
        if (!editid) {
            api1 = '<?=$api_base_url?>pathology-generate-bill';
            method = "POST";
            api2 = '<?=$api_base_url?>pathology-generate-bill/pathologyReport';
        } else {
            api1 = '<?=$api_base_url?>pathology-generate-bill/' + editid;
            method = "PATCH";
            api2 = '<?=$api_base_url?>pathology-generate-bill/editpathologyReport';
        }
        ...
        pathologyItems.forEach((item, index) => {
        ...
        $.ajax({
            ...
            success: function(data) {
                let id = data?.[0]?.["data "]?.Pathology_bill_Values?.[0]?.id 
                || data?.[0]?.["data "]?.updated_values?.[0]?.id 
                || null;  

                pathologyItems.forEach(function(item) {
                    item.pathology_bill_id = id;
                });
                console.log(JSON.stringify(pathologyItems,null,2));
...
<script type="text/javascript">
    ...
    $(document).on('submit','#add_partial_payment', function(e){
      ...
      $.ajax({
          ...
          $(document).on('click','.delete_trans', function(e){
            ...
            btn.button('loading');
            $.ajax({
                url:'<?=$api_base_url?>pathology-generate-bill/deletePathoPayment/'+record_id+'?Hospital_id=<?=$data['hospital_id']?>',
                type: "DELETE",
   ...
   function popup(data)
    ...
    var form = $(this);    
    var formDataJson = {
        "collection_specialist": $('#collected_by').val(),  
        "collection_date": $(this).find('input[name="collected_date"]').val(), 
        "pathology_center": $(this).find('input[name="pathology_center"]').val(),
        "hospital_id": <?=$data['hospital_id']?>
    };   
    var dateCollected = $(this).find('input[name="collected_date"]').val();
    formDataJson.collection_date = moment(dateCollected, 'MM/DD/YYYY').format('YYYY-MM-DD');   
...
$(document).on('submit', '#form-report_param', function(e) {
    ...
    var formattedData = {
        ...
        "pathology_report_value": formData.get('pathology_report_value'),
    };
    ...
    if (fileInput) {
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                formattedData.pathology_report = formattedData.documents;
    ...
    function submitForm(data) {
        var pathology_bill_id=$('#patho_bill_id').val();
        $.ajax({
            url: '<?=$api_base_url?>pathology-generate-bill/updateApprovalByPerson/' + pathology_bill_id,
            type: "PATCH",
            ...
            success: function(response) {
                let message = response[0]?.data?.message || 'Default success message';
                successMsg(message); 
                location.reload();
            },
            error: function(xhr) {

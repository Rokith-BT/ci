                ...
                <div class="box box-primary">
                    <div class="box-header with-border">
                        ...
                        <div class="box-tools pull-right">
                            ...
                            ?>
                                <a data-toggle="modal" onclick="holdModal('myModal')" id="add_charge_modal"
                                    class="btn btn-primary btn-sm charge"><i class="fa fa-plus"></i>
                                    <?php echo $this->lang->line('add_charges'); ?></a>
                            <?php } ?>
                    ...
                    <div class="row">
                            ...
                            <div class="form-group">
                                ...
                                <select name="charge_type" class="form-control charge_type">
                                    ...
                                    <?php foreach ($charge_type as $charge_key => $charge_value) { ?>
                                        <option value="<?php echo $charge_value->id; ?>">
                                            <?php echo $charge_value->charge_type; ?></option>
                                    <?php } ?>
                                </select>
                        ...
                        <div class="col-sm-3">
                            <div class="form-group">
                                ...
                                <small class="req"> *</small>
                                <select name="charge_category_id" id="charge_category" class="form-control select2"
                                    style="width: 100%">
                                    <option value=""><?php echo $this->lang->line('select') ?></option>
                                </select>
                        ...
                        <div class="col-md-2">
                            <div class="form-group">
                                ...
                                <select name="charge_unit_id" class="form-control unit_type" id='charge_unit_id'>
                                    <option value=""><?php echo $this->lang->line('select') ?></option>
                                    <?php foreach ($unit_type as $unit_type_key => $unit_type_value) {
                                    ?>
                                        <option value="<?php echo $unit_type_value->id; ?>">
                                            <?php echo $unit_type_value->unit; ?></option>
                                    <?php } ?>
                                </select>
                        ...
                        <div class="col-md-4">
                            <div class="form-group">
                                <label><?php echo $this->lang->line('charge_name') ?></label><small class="req">
                                    *</small>
                                <input type="text" name="name" id="charge_name" class="form-control">
                        ...
                        <div class="col-md-6">
                            <div class="row">
                                <div class="col-sm-6">
                                    <div class="form-group">
                                        <label><?php echo $this->lang->line('tax_category'); ?></label><small
                                            class="req"> *</small>
                                        <select name="tax_category_id" id="taxcategory"
                                            class="form-control tax_category">
                                            <option value=""><?php echo $this->lang->line('select') ?></option>
                                            <?php foreach ($taxcategory as $taxcategory_key => $taxcategory_value) {
                                            ?>
                                                <option value="<?php echo $taxcategory_value['id']; ?>">
                                                    <?php echo $taxcategory_value['name']; ?></option>
                                            <?php } ?>
                                        </select>
                                ...
                                <div class="col-sm-6">
                                    <div class="form-group">
                                        ...
                                        <div class="input-group">
                                            <input type="text" class="form-control right-border-none" readonly
                                                name="tax_percentage" id="tax_percentage" autocomplete="off">
                            ...
                            <div class="form-group">
                                <label><?php echo $this->lang->line('standard_charge') . " (" . $currency_symbol . ")"; ?></label><small
                                    class="req"> *</small>
                        ...
                        <div class="col-sm-12 col-md-6 col-lg-6">
                            ...
                            <button type="button" class="plusign"
                                onclick="apply_to_all()"><?php echo $this->lang->line('apply_to_all'); ?></button>
                            <div class="chargesborbg">
                                <div class="form-group">
                                    <table class="printablea4">
                                        <?php foreach ($schedule as $category) { ?>
                                            <tr id="schedule_charge">
                                                <input type="hidden" name="schedule_charge_id[]"
                                                    value="<?php echo $category['id']; ?>">
                                                <td class="col-sm-8"
                                                    style="vertical-align: bottom; text-align: left; padding-right: 20px;">
                                                    <?php echo $category['organisation_name']; ?></td>
                                                <td class="col-sm-4">
                                                    <input type="text" name="schedule_charge_<?php echo $category['id']; ?>"
                                                        id="schedule_charge_<?php echo $category['id']; ?>"
    ...
    $(document).on('change', '.tax_category', function() {
        ...
        $.ajax({
            ...
            success: function(data) {
                $('input[name="tax_percentage"]').val(data.percentage);
            },
            error: function() {},
            complete: function() {
        ...
        $.ajax({
            ...
            success: function(data) {
                if (data.status == 0) {
                    ...
                    $('.charge_type option[value="' + data.result.charge_type_master_id + '"]').prop(
                        'selected', true);
                    $('.unit_type option[value="' + data.result.charge_unit_id + '"]').prop('selected',
                        true);
                    ...
                    getcharge_category(data.result.charge_type_master_id, data.result
                        .charge_category_id);
                    $('#myModal').modal('show');
                }
                btn.button('reset');
    ...
    function deleteChargeType(url) {
        ...
        if (confirm(msg)) {
            ...
            $.ajax({
                ...
                beforeSend: function() {},
                success: function(res) {
                    successMsg(response[0]?.['data ']?.messege || 'Default success message');
...
<script>
    $(document).ready(function() {
        $('#formadd').submit(async function(e) {
            e.preventDefault();
            try {
                const chargeCategory = $('#charge_category').val().trim();
                const chargeName = $('#charge_name').val().trim();
                const chargeUnitId = $('#charge_unit_id').val().trim();
                const taxCategory = $('#taxcategory').val().trim();
                const standardCharge = $('#standard_charge').val().trim();
                let errorMessages = [];
                if (!chargeCategory) errorMessages.push('Charge Category is required');
                if (!chargeName) errorMessages.push('Charge Name is required');
                if (!chargeUnitId) errorMessages.push('Unit Type is required');
                if (!taxCategory) errorMessages.push('Tax Category is required');
                if (!standardCharge) errorMessages.push('Standard Charge is required');
                ...
                let apiurl = '<?= $api_base_url ?>setup-hospital-charge-charges';
                let method = 'POST';
                const id = $('#id').val();
                if (id && id != 0) {
                    apiurl += '/' + id;
                ...
                sendAjaxRequest(apiurl, method, jsonData, response => {
...
<script>
    ...
    $(document).ready(function() {
        let actionTemplate = `
        <a href="#" onclick="viewDetail(key:id)" class="btn btn-default btn-xs" data-toggle="tooltip" title="Show">
            <i class="fa fa-reorder"></i>
        </a>
        <a href="javascript:void(0)" class="btn btn-default btn-xs edit_record edit_charge_modal" data-loading-text="Please Wait.." data-toggle="tooltip" data-record-id="key:id" title="Edit">
            <i class="fa fa-pencil"></i>
        </a>
    `;
        initializeTable(initialData, initialDataTotal, `${base_url}admin/charges/getchargelist`, '#ajaxlist', ['sno',
                'name', 'charge_category', 'charge_type', 'unit', 'tax', 'standard_charge', 'action'
            ],
            actionTemplate,
            'id'
        );

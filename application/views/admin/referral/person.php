...
$(document).ready(function (e) {
    $('#addperson').on('submit', (function (e) {
        ...
        var jsonData = {
            ...
            contact: $("input[name='referrer_contact']").val(),
            person_name: $("input[name='person_name']").val(),
            person_phone: $("input[name='person_phone']").val(),
            category_id: parseInt($("select[name='category']").val()),
            standard_commission: parseFloat($("input[name='commission']").val()),
        ...
        $.ajax({
            url: '<?=$api_base_url?>referral-person',
            type: "POST",
            data: jsonData,
            dataType: 'json',
            success: function (response) {
                successMsg(response[0]["data "].messege || 'Successfully');
                location.reload();
                $("#addpersonbtn").button('reset');
            },
            error: function () {
    ...
    function validateForm() {
        ...
        if (!$("input[name='name']").val()) {
            errorMessages.push('<li>Referrer Name is required.</li>');
            isValid = false;
        }

        if (!$("input[name='referrer_contact']").val()) {
            errorMessages.push('<li>Referrer Contact is required.</li>');
            isValid = false;
        }

        if (!$("input[name='person_name']").val()) {
            errorMessages.push('<li>Contact Person Name is required.</li>');
            isValid = false;
        }

        if (!$("input[name='person_phone']").val()) {
            errorMessages.push('<li>Contact Person Phone is required.</li>');
            isValid = false;
        }

        if (!$("select[name='category']").val()) {
            errorMessages.push('<li>Category is required.</li>');
            isValid = false;
        }

        if (!$("input[name='commission']").val() || isNaN($("input[name='commission']").val())) {
            errorMessages.push('<li>Standard Commission is required and must be a valid number.</li>');
            isValid = false;
        }

        if (!$("input[name='address']").val()) {
            errorMessages.push('<li>Address is required.</li>');
            isValid = false;
        }

        if (!isValid) {
            errorMsg('<ul>' + errorMessages.join('') + '</ul>');
        }
    ...
    $('#editperson').on('submit', (function (e) {
        ...
        var jsonData = {
            ...
            "contact": formData.get("referrer_contact"),
            "person_name": formData.get("person_name"),
            "person_phone": formData.get("person_phone"),
            "category_id": parseInt(formData.get("category")),
            "standard_commission": parseFloat(formData.get("commission")),
            "address": formData.get("address"),
            "Hospital_id": <?=$data['hospital_id']?>
        };

        // console.log(JSON.stringify(jsonData));
        let editid = $("#person_id").val();
        $.ajax({
            url: '<?=$api_base_url?>referral-person/' + editid,
            type: "PATCH",
...
<script type="text/javascript">
    ...
    if(conf == true){
        $.ajax({
            url: '<?=$api_base_url?>referral-person/' + id + '?Hospital_id=<?=$data['hospital_id']?>',
            type: "Delete",
            dataType: "json",
            success: function (data) {

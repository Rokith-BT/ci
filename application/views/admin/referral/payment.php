...
$(document).ready(function (e) {
    $('#addpayment').on('submit', function (e) {
        ...
        object["referral_person_id"] = parseInt(formData.get("payee")); 
        object["patient_id"] = parseInt(formData.get("patient_id"));
        object["referral_type"] = parseInt(formData.get("patient_type")); 
        object["billing_id"] = parseInt(formData.get("bill_no")); 
        object["bill_amount"] = parseFloat(formData.get("bill_amount"));
        object["percentage"] = parseFloat(formData.get("percentage"));
        object["amount"] = parseFloat(formData.get("commission_amount")); 
        object["Hospital_id"] = <?=$data['hospital_id']?>;
        var json = JSON.stringify(object);
        console.log(json);       
        $.ajax({
            url: '<?=$api_base_url?>referral-payment',
            type: "POST",
            data: json,  
            dataType: 'json',
    ...
    $('#editpayment').on('submit', function (e) {
        ...
        jsonData.percentage = formData.get("commission_percentage");
        jsonData.amount = formData.get("commission_amount");        
        jsonData.Hospital_id = <?=$data['hospital_id']?>;  
        let paymentid = formData.get("paymentid"); 

        console.log(JSON.stringify(jsonData));       
        $.ajax({
            url: '<?=$api_base_url?>referral-payment/' + paymentid,
            type: "PATCH",
...
$('#addpatient_id').on('select2:select', function (e) {
    let id = $(this).val();
    var base_url = "<?php echo base_url(); ?>";  // Moved base_url declaration to the top
    ...
    $.ajax({
        ...
        dataType: 'json',
        beforeSend: function () {
            $("#ajax_load").html("<center><img src='" + base_url + "backend/images/loading.gif'/>");
        },
        success: function (res) {
            if (res) {
                ...
                $("#identification_number").html(res.ABHA_number);
                ...
                // Fetch image using external API
                $.ajax({
                    url: 'https://phr-api.plenome.com/file_upload/getDocs',
                    type: "POST",
                    contentType: "application/json",
                    data: JSON.stringify({ value: res.image }),
                    success: function (decryptResponse) {
                        let base64Image = "data:image/png;base64," + decryptResponse;
                        if (decryptResponse && typeof decryptResponse === 'string' && decryptResponse.length > 0) {                          
                            $("#image").attr("src", base64Image);
                        } else {                          
                            $("#image").attr("src", base_url + res.image + '<?php echo img_time(); ?>');
                        }
                    },
                    error: function () {                      
                        $("#image").attr("src", base_url + res.image + '<?php echo img_time(); ?>');

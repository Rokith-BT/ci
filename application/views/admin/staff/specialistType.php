...
<div class="modal fade" id="myModal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
    <div class="modal-dialog modal-mid" role="document">
        <div class="modal-content modal-media-content">
            ...
            <form id="formadd"  name="employeeform" method="post" accept-charset="utf-8"  enctype="multipart/form-data">
...
$(document).ready(function() {
    $('#formadd').on('submit', function(e) {
        e.preventDefault();
        let specialist_name = $('#type').val();
        if (!specialist_name || specialist_name.trim() === '' || !/^[A-Za-z0-9\s]+$/.test(specialist_name)) {
            errorMsg('Please enter a valid Specialist name. It should not contain only spaces or special characters.');
            return false;
        }
        var formData = {
            specialist_name: $('#type').val(),
            Hospital_id: <?=$data['hospital_id']?>,
            is_active :'yes'
        };
          sendAjaxRequest('<?=$api_base_url?>setup-human-resource-specialist', 'POST', formData,function(response) {
...
<script>
    $(document).ready(function() {
        $('#editformadd').on('submit', function(e) {
            e.preventDefault();
            let specialist_name = $('#type1').val();
            if (!specialist_name || specialist_name.trim() === '' || !/^[A-Za-z0-9\s]+$/.test(specialist_name)) {
                errorMsg('Please enter a valid Specialist name. It should not contain only spaces or special characters.');
                return false;
            }
            var formData = {
                'specialist_name': $('#type1').val(),
                "Hospital_id":<?=$data['hospital_id']?>,
            };
            let  id = $('#dept_id').val();
            sendAjaxRequest('<?=$api_base_url?>setup-human-resource-specialist/' + id, 'PATCH', formData, function(response) {
                handleResponse(response);
            });
        });
    });
...
$(document).ready(function() {
    ...
    `;
    initializeTable(initialData, initialDataTotal, `${base_url}admin/specialist/getspecialistlist`, '#ajaxlist', [
            'sno','specialist_name', 'action'
        ],
        actionTemplate,
        'id'
    );
...
<script>
    ...
    function deleterecord(id)
    {
        if(confirm('Are you sure you want to delete this')){
            sendAjaxRequest('<?=$api_base_url?>setup-human-resource-specialist/' + id + '?hospital_id=<?=$data['hospital_id']?>', 'DELETE', {}, function(response) {
                handleResponse(response);
            });
        }
    }

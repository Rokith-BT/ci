...
function fetchStaffData(page = 1) {
    ...
    $.ajax({
        url: "<?= base_url('admin/staff/Getstafffdetials') ?>",
        type: "POST",
        data: postData,
        dataType: "json",
...
$(document).ready(function() {
    ...
    const table = $(Tableid).DataTable({
        ...
        ajax: function(data, callback) {
            ...
            fetch(`${baseurl}admin/staff/Getstafffdetials`, {
                method: 'POST',
                headers: {
...
function renderTable(dataArray, recordCount, data, callback) {
    ...
    const rows = (dataArray || []).map(item => {
        const action = `<td class="pull-right noExport">
            <a href="<?= base_url('admin/staff/edit/') ?>${item.id}" class="btn btn-default btn-xs" data-toggle="tooltip" title="Edit"><i class="fa fa-pencil"></i></a>

        ...
        <div class="row">
            <div class="col-md-12">
                <div class="box box-primary">
                    ...
                    <form id="employeeform1" name="employeeform"  enctype="multipart/form-data">
                        <div class="box-body">
                            <div class="row">
                                <div class="col-md-6">
                                    <div class="form-group">
                                        <label for="medicine_category_id"><?php echo $this->lang->line('medicine_category'); ?></label><small class="req"> *</small>
                                        <select id="medicine_category_id" name="medicine_category_id" class="form-control">
                                            <option value=""><?php echo $this->lang->line('select'); ?></option>
                                            <?php foreach ($medicineCategory as $medicine) { ?>
                                ...
                                <div class="col-md-12 pt20">
                                    <button type="submit" class="btn btn-info btn-sm pull-right">
                                        <i class="fa fa-upload"></i> <?php echo $this->lang->line('import_medicine'); ?>
                                    </button>
                                </div>
                            </div>
                        </div>
...
<script>
    $('#employeeform1').on('submit', function (e) {
        e.preventDefault();
        ...
        if (fileInput) {
            Papa.parse(fileInput, {
                ...
                complete: function (results) {
                    var formattedData = results.data.map(function (item) {
                        return {
                            "medicine_name": item.medicine_name || "",
                            "medicine_category_id": $('#medicine_category_id').val(),
                            "medicine_image": "Medicaimage",
                            "medicine_company": item.medicine_company || "",
                            "medicine_composition": item.medicine_composition || "",
                            "medicine_group": item.medicine_group || "",
                            "unit": item.unit || "",
                            "min_level": item.min_level || "normal",
                            "reorder_level": item.reorder_level || "normal",
                            "vat": parseFloat(item.vat) || 10.00,
                    ...
                    // Use Promise.all to ensure all requests are completed
                    var ajaxPromises = formattedData.map(function (medicineData) {
                        return $.ajax({
                    ...
                    Promise.all(ajaxPromises)
                        .then(function (responses) {
                            console.log('All data successfully inserted:', responses);
                            $('#employeeform1').trigger('reset');
                        })

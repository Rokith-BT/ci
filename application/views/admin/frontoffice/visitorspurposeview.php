                                            ...
                                            <tr>
                                                ...
                                                <td class="mailbox-date pull-right">
                                                    ...
                                                    <?php if ($this->rbac->hasPrivilege('setup_front_office', 'can_delete')) {?>
                                                        <a class="btn btn-default btn-xs" data-toggle="tooltip" title="" onclick="delete_recordByIdReload('<?php echo $api_base_url; ?>setup-front-office-purpose/<?php echo $value['id']; ?>?Hospital_id=<?php echo $data['hospital_id']; ?>', '<?php echo $this->lang->line('delete_confirm'); ?>')" data-original-title="<?php echo $this->lang->line('delete'); ?>">
...
<div class="modal fade" id="myModal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
    <div class="modal-dialog modal-mid" role="document">
        <div class="modal-content modal-media-content">
            ...
            <form id="visitorspurpose" name="employeeform" method="post" accept-charset="utf-8">
...
$(document).ready(function() {
    $('#visitorspurpose').on('submit', function(e) {
        e.preventDefault();
        var formData = {
            visitors_purpose:$('#visitors_purpose').val(),
            description:$('#description').val(),
            Hospital_id:'<?=$data['hospital_id']?>'    
        };
        console.log(formData);
        $.ajax({
            url: '<?=$api_base_url?>setup-front-office-purpose', 
            type: 'POST',
            data: formData,
            dataType: 'json',
            beforeSend: function() {
            ...
            success: function(response) {
                ...
                successMsg('Purpose details added successfully'); 
                location.reload();
            },
        ...
        <div class="modal-content modal-media-content">
            ...
            <form id="editformadd"  name="employeeform" method="post" accept-charset="utf-8"  enctype="multipart/form-data" class="ptt10">
                <div class="modal-body pt0 pb0">
                    <div class="form-group">
                            <label for="pwd"><?php echo $this->lang->line('purpose'); ?></label> <small class="req"> *</small>
                            <input class="form-control" id="visitors_purpose1" name="visitors_purpose" />
                            <span class="text-danger"><?php echo form_error('visitors_purpose'); ?></span>
                        </div>
                        <div class="form-group">
                            <label for="pwd"><?php echo $this->lang->line('description'); ?></label>
...
<script>
    $(document).ready(function() {
        $('#editformadd').on('submit', function(e) {
            ...
            let id= $('#id').val()
            var formData = {
                visitors_purpose:$('#visitors_purpose1').val(),
                description:$('#description1').val(),
                id:$('#id').val()
            };
            console.log(formData);
            $.ajax({
                url: '<?=$api_base_url?>setup-front-office-purpose/'+id, 
                type: 'PATCH',
                data: formData,
                dataType: 'json',
                beforeSend: function() {
                ...
                success: function(response) {
                    ...
                    successMsg('Purpose details updated successfully'); 
                    location.reload();
                },
                error: function(xhr, status, error) {
                    $('#editformaddbtn').button('reset');
    ...
    function get(id) {
        ...
        $.ajax({
            ...
            success: function (result) {
                $('#id').val(result.id);
                $('input[name="visitors_purpose"]').val(result.visitors_purpose);

                ...
                <div class="scroll-area">
                    <div class="modal-body pt0 pb0">
                                        ...
                                        <div class="bootstrap-timepicker">
                                            <div class="form-group">
                                                <div class="input-group">
                                                    <input type="text" name="in_time" class="form-control timepicker" id="in_time" value="<?php echo set_value('time'); ?>">
                            ...
                            <div class="form-group">
                                ...
                                <div>
                                    <input class="filestyle form-control" type='file' name='file' id="edit_attachment_report"  />
                                    <input class="" type='hidden' name='file' id="old_attachment_report"  />
                                </div>
...
<div id="myModal" class="modal fade" role="dialog">
    <div class="modal-dialog">
        ...
        <div class="modal-content">
            ...
            <form id="formadd"  method="post" accept-charset="utf-8" enctype="multipart/form-data" class="ptt10">
                <div class="scroll-area">
                    <div class="modal-body pt0 pb0">
                            <div class="row">
                                <div class="col-md-6">
                                ...
                                <div class="col-md-6">
                                    <div class="form-group">
                                        ...
                                        <div class="bootstrap-timepicker">
                                            <div class="form-group">
                                                <div class="input-group">
                                                    <input type="text" name="in_time" class="form-control timepicker" id="stime_" value="<?php echo set_value('time'); ?>">
                                                    <div class="input-group-addon">
                                                        <i class="fa fa-clock-o"></i>
                                                    </div>
                                                </div>
...
function get(id) {
    ...
    $.ajax({
        ...
        success: function (result) {
            ...
            $('#old_attachment_report').val(result.image);
...
function deletevisitor(id){
    ...
    if(id !=''){
        if (confirm("<?php echo $this->lang->line('are_you_sure_to_delete_this'); ?>")){
            $.ajax({
            url: '<?=$api_base_url?>visitor-book/removeFrontofficeVisitors/'+id+'?hospital_id=<?=$data['hospital_id']?>',
            type: 'Delete',
            data:{id:id},
...
<script>
    ...
    $("#formadd").on('submit', function (e) {
        ...
        var dateInput = formData.get('date'); 
        var hourInput = formData.get('hour'); 
        var minuteInput = formData.get('minute'); 
        var meridianInput = formData.get('meridian');

        var formattedDate = formatDateTime(dateInput, hourInput, minuteInput, meridianInput);       
        ...
        var jsonData = {
            ...
            "visit_to": formData.get('visit_to'),
            "ipd_opd_staff_id": formData.get('ipd_opd_staff'),
            "related_to": formData.get('related_to'),
            "no_of_pepple": formData.get('pepples'),
            "date": formattedDate,
            "in_time": formatTime(formData.get('in_time')),
            "out_time": formatTime(formData.get('out_time')),
        ...
        if (fileInput) {
            ...
            $.ajax({
                ...
                success: function(response) {
                    jsonData.image = response.data;
                    submitForm(jsonData);
                },
                error: function() {
                    jsonData.image = documentUrl;
                    submitForm(jsonData);
                }
            });
        } else {
            jsonData.image = 'none.img';
            submitForm(jsonData);
        }
        ...
        function formatTime(time) {
            return time.replace(/(\d+):(\d+) (\w+)/, (match, hours, minutes, modifier) => {
                const adjustedHours = modifier === 'PM' && hours < 12 ? parseInt(hours) + 12 : (modifier === 'AM' && hours == 12 ? 0 : parseInt(hours));
                return `${String(adjustedHours).padStart(2, '0')}:${String(minutes).padStart(2, '0')}:00`;
            });
        }
        function formatDateTime(date, hour, minute, meridian) {
            var dateParts = date.split("/");
            var formattedDate = dateParts[2] + "-" + ("0" + dateParts[0]).slice(-2) + "-" + ("0" + dateParts[1]).slice(-2);
            return formattedDate + " " + formatTime(`${hour}:${minute} ${meridian}`);
        }
...
$(document).ready(function (e) {
    $("#editformadd").on('submit', function (e) {
        ...
        let fileInput = document.getElementById('edit_attachment_report').files[0];
        var documentUrl = $('#filedataalread').val();
        ...
        if (fileInput) {
            ...
            jsonData.image = $('#old_attachment_report').val();
            submitEditForm(jsonData);
        }

        function submitEditForm(jsonData) {
            console.log(JSON.stringify(jsonData, null, 2));
            let editid = $('#editid').val();
            $.ajax({
                url: '<?=$api_base_url?>visitor-book/' + editid,
                type: "PATCH",
                data: JSON.stringify(jsonData),
                dataType: 'json',
                contentType: 'application/json; charset=utf-8',
                ...
                error: function() {
                    alert("Error updating the visitor. Please try again.");
                }
            });
        }

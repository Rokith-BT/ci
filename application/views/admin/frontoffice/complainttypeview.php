                                            ...
                                            <tr>
                                                ...
                                                <td class="mailbox-date pull-right">
                                                    ...
                                                    <?php }if ($this->rbac->hasPrivilege('setup_front_office', 'can_delete')) {?>
                                                        <a class="btn btn-default btn-xs" data-toggle="tooltip" title="" onclick="delete_complainttype('<?php echo $api_base_url; ?>setup-front-office-complain-type/<?php echo $value['id']; ?>?Hospital_id=<?php echo $data['hospital_id']; ?>')" data-original-title="<?php echo $this->lang->line('delete'); ?>">
...
<div class="modal fade" id="myModal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
    <div class="modal-dialog modal-mid" role="document">
        ...
        <form   id="complainttypeviewadd" name="employeeform" method="post" accept-charset="utf-8">
...
$(document).ready(function() {
    $('#complainttypeviewadd').on('submit', function(e) {
        e.preventDefault();
        var formData = {
        ...
        $.ajax({
            url: '<?=$api_base_url?>setup-front-office-complain-type',
            type: 'POST',
            data: formData,
            dataType: 'json',
            beforeSend: function() {
            ...
            success: function(response) {
                ...
                successMsg('Complaint Type add successfully!');
                location.reload();
            },
            ...
            <form id="editformadd"  name="employeeform" method="post" accept-charset="utf-8"  enctype="multipart/form-data">
                <div class="modal-body pt0 pb0">
                    <div class="ptt10">
                        <div class="form-group">
                            <label for="pwd"><?php echo $this->lang->line('complain_type'); ?></label><small class="req"> *</small>
                            <input class="form-control" id="complaint_type1" name="complaint_type"  value="<?php echo set_value('complaint_type'); ?>"/>
                            <span class="text-danger"><?php echo form_error('complaint_type'); ?></span>
                        </div>
                        <div class="form-group">
                            <label for="pwd"><?php echo $this->lang->line('description'); ?></label>
...
$(document).ready(function(){
    $("#editformadd").on('submit', function(e){
        ...
        var formData = {
            complaint_type: $('#complaint_type1').val(),
        ...
        $.ajax({
            url: '<?=$api_base_url?>setup-front-office-complain-type/' + id,
            type: 'PATCH',
            data: data,
    ...
    function get(id) {
        ...
        $.ajax({
            ...
            success: function (result) {
                $('#id').val(result.id);
                $('input[name="complaint_type"]').val(result.complaint_type);

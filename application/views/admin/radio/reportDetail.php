            ...
            <div class="modal-header modal-media-header">
                ...
                <div class="row">
                    <div class="col-lg-5 col-md-5 col-sm-5 col-xs-9">
                        ...
                        <select name="patientid" class="form-control patient_list_ajax" id="addpatient_id">
     ...
     $(document).on('click','.assigntest',function(){
      ...
      function addTotal() {
                ...
                medicineTable.find("tbody tr").each(function() {
                    var amount = parseFloat($(this).find("td input.amount").val()) || 0;
                    var taxAmount = parseFloat($(this).find("td input.taxamount").val()) || 0;
                ...
                if (!testSelected) {
                    errorMsg("Select at least one test.");
                    return;
                }
                ...
                if (total > 0) {
                    ...
                    var net_amount = total + total_taxamt - discount;
...
function getPrescriptionData(){
  ...
  $.ajax({
   ...
   function getPayments(record_id){
    ...
    var radiology_billing_id = $("input[name='radiology_billing_id']").val();
    var form = $(this);
    var btn = clicked_btn;
    btn.button('loading');

    var formData = new FormData(this);
    var paymentMode = formData.get('payment_mode') || 'online';

    var jsonData = {
        "payment_date": '<?= date('Y-m-d H:i:s'); ?>',
        "patient_id": formData.get('patient_id') || '',
        "radiology_billing_id": radiology_billing_id || '',
        "note": formData.get('note') || 'paid',
        "received_by": <?= $data["id"] ?> || '',
    ...
    if (jsonData.amount == 0) {
        errorMsg('Payment must be greater than 0!');
        btn.button('reset');
        return;
    }
    ...
    if (paymentMode === 'Online') {
        ...
        jsonData.cheque_no = formData.get('cheque_no') || '';
        jsonData.cheque_date = formData.get('cheque_date') || '';
        jsonData.attachment = formData.get('attachment') || '';
        jsonData.attachment_name = formData.get('attachment_name') || '';
    }else{
        submitForm(jsonData);
    }

    console.log("Form Data in JSON format:", JSON.stringify(jsonData, null, 2));

    function submitForm(jsonData){
        $.ajax({
            url: "<?=$api_base_url?>radiology-generate-bill/AddRadiologyPayment",
            type: "POST",
            ...
            success: function (data) {
                if (data.status === "fail") {
                    var message = data.error.join(" ");
                    errorMsg(message);
                } else {
                    $("#addPaymentModal").modal('hide');
                    successMsg(data[0]["data "].messege);
                    getPayments(data[0]["data "].Added_Payment_values[0].id);
                    table.ajax.reload();
                }
                btn.button('reset');
            },
...
<script>
    ...
    $(document).on('submit', '#bill', function (e) {
        ...
        let tax_percentage = formData.get('tax') || '';
        ...
        if(doctor_name=='')
        {
           errorarray.push('Please Select Doctor');            
        }else if(patient_id=='')
        {
            errorarray.push('Please Select Patient');
        }        
        ...
        let data1 = {
            ...
            "ipd_prescription_basic_id": ipd_prescription_basic_id || undefined,
            ...
            "tax": tax || undefined,
            "net_amount": net_amount || undefined,
            "note": note || undefined,
        ...
        if (payment_mode === 'Cheque') {
            if (cheque_no) data1.cheque_no = cheque_no;
            if (cheque_date) data1.cheque_date = cheque_date;
            if (attachment) data1.attachment = attachment;
        } else if (payment_mode === 'Online') {
            handlePayment(data1).then((paymentSuccess) => {
                if (paymentSuccess) {
                    jsonData.payment_gateway = 'razorpay';
                    jsonData.payment_id = paymentSuccess.payment_id;
                    jsonData.payment_reference_number = paymentSuccess.reference_id;
        ...
        function submitForm(jsonData) {
            ...
            $.ajax({
                ...
                success: function (response) {
                    let radiology_bill_id = response?.[0]?.["data "]?.Radiology_bill_Values?.[0]?.id || response?.[0]?.["data "]?.updated_values?.[0]?.id;
                    ...
                    for (let i = 1; i <= totalRows; i++) {
                        ...
                        if (testName && reportDate && taxPercent && amount) {
                            let radiologyTest = {
                                "radiology_bill_id": radiology_bill_id || undefined,
                                "radiology_id": testName || undefined,
                                "patient_id": patient_id || undefined,
                                "reporting_date": `${formattedDate} 11:11:11`,
                                "tax_percentage": taxPercent || 0,
                                "apply_charge": amount || undefined,
                                "hospital_id": hospital_id || undefined,
                                "consultant_doctor": doctor_name || undefined,
                                "generated_by": doctor_id || undefined
                            };

                            if (edit) {
                                let inserted_id = $("input[name='inserted_id_" + i + "']").val();
                                if (inserted_id) radiologyTest.id = inserted_id;
                            }
                    ...
                    $.ajax({
                        ...
                        success: function (data) {
                            ...
                            if (submit_button_name == "save_print") {
                                let insertId = data?.data?.[0]?.RadiologyReportDetail?.id;
                                if (insertId) {
                                    printData(insertId);
                                }
                            }

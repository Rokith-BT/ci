...
<script>
        ...
        $("#formadd").on('submit', function(e) {
            ...
            $(this).find(':input[name]').each(function() {
                ...
                if (input.attr('type') === 'file') {
                    formDataObj[input.attr('name')] = input[0].files.length > 0 ? input[0].files[0].name : null;
                } else {
                    formDataObj[input.attr('name')] = input.val().trim();
                }
            });

            let missingFields = [];
            if (!formDataObj.donor_name) missingFields.push("Donor Name");
            if (!formDataObj.date_of_birth) missingFields.push("Date of Birth");
            if (!formDataObj.blood_group) missingFields.push("Blood Group");
            if (!formDataObj.gender) missingFields.push("Gender");

            if (missingFields.length > 0) {
                const requiredErrorMsg = "Please fill all required fields: " + missingFields.join(", ");
            ...
            const final_data = {
                "donor_name": formDataObj.donor_name,
                "date_of_birth": convertDateToYYYYMMDD(formDataObj.date_of_birth),
                "blood_bank_product_id": formDataObj.blood_group,
                "gender": formDataObj.gender,
                "father_name": formDataObj.father_name,
                "address": formDataObj.address,
                "contact_no": formDataObj.contact_no,
                "hospital_id": <?= $data['hospital_id'] ?>
            };

            $.ajax({
                url: '<?= $api_base_url ?>donor-details',
                type: "POST",
    ...
    $(document).ready(function(e) {
        $("#formedit").on('submit', (function(e) {
            ...
            $.ajax({
                url: '<?= $api_base_url ?>donor-details/' + id,
                type: "PATCH",
                data: JSON.stringify(final_data, null, 2),
                dataType: 'json',
                contentType: 'application/json',
                cache: false,
                processData: false,
        ...
        $.ajax({
            ...
            success: function(data) {
                ...
                $("#edit_delete").html("<a href='#' onclick='printData(" + id + ")' data-placement='bottom' data-toggle='tooltip' title='' data-original-title='<?php echo $this->lang->line('print') ?>'><i class='fa fa-print'></i></a><?php if ($this->rbac->hasPrivilege('blood_donor', 'can_edit')) { ?><a href='#' onclick='getRecord(" + id + ")' data-toggle='tooltip' data-placement='bottom' title='' data-original-title='Edit'><i class='fa fa-pencil'></i></a><?php }
                                                                                                                                                                                                                                                                                                                                                                                                                                                                        if ($this->rbac->hasPrivilege('blood_donor', 'can_delete')) { ?><a onclick='delete_record(" + id + ")'  href='#'  data-toggle='tooltip' data-placement='bottom'  data-original-title='Delete'><i class='fa fa-trash'></i></a><?php } ?>");
                holdModal('viewModal');
            },
        });
    }
        ...
        $("#donorblood").on('submit', function(e) {
            ...
            $(this).find(':input[name]').each(function() {
                ...
                if (input.attr('type') === 'file') {
                    ...
                    formDataObj[input.attr('name')] = input.val();
                }
            });
            if (!formDataObj.donate_date) {
                errorMsg('Donate Date is requried');
                return false;
            }
            if (!formDataObj.bag_no) {
                errorMsg('Bag No is requried');
                return false;
            }
            if (!formDataObj.standard_charge) {
                errorMsg('Standard Charge is requried');
                return false;
            }
            const finalData = {
                "blood_donor_cycle_id": 0,
                "blood_bank_product_id": formDataObj.blood_bank_product,
                "blood_donor_id": formDataObj.blood_donor_id,
                "charge_id": formDataObj.charge_id,
                "donate_date": convertDateToYYYYMMDD(formDataObj.donate_date),
                "bag_no": formDataObj.bag_no,
                "lot": formDataObj.lot,
                "quantity": formDataObj.qty,
                "standard_charge": formDataObj.standard_charge,
                "apply_charge": formDataObj.apply_charge,
                "amount": formDataObj.amount,
                "institution": formDataObj.institution,
                "note": formDataObj.note,
                "discount_percentage": formDataObj.discount_percent,
                "tax_percentage": formDataObj.tax_percentage,
            ...
            $.ajax({
                url: '<?= $api_base_url ?>donor-details/bloodDonorCycle',
                type: "POST",

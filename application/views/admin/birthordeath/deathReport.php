...
<script type="text/javascript">
    ...
    $("#formadd").on('submit', function (e) {
        ...
        const requiredFields = ['case_id', 'patient', 'death_date', 'guardian_name'];
        ...
        let finalobject = {
            "patient_id": parseInt(formObject.patient),
            "case_reference_id": parseInt(formObject.case_id),
            "attachment": "",
            "attachment_name": "",
            "death_date": formatDate(formObject.death_date),
            "guardian_name": formObject.guardian_name ? formObject.guardian_name : "noyaliyin poruppaalar",
            "death_report": formObject.death_report ? formObject.death_report : "summa saavaduchaachu",
        ...
        if (fileInput) {
            ...
            $.ajax({
                ...
                success: function (response) {
                    finalobject.attachment = response.data;
                    finalobject.attachment_name = fileInput.name;
                ...
                error: function (xhr, status, error) {
                    ...
                    finalobject.attachment = documentUrl;
                    console.log(JSON.stringify(finalobject, null, 2));
                    $("#formaddbtn").button('reset');
                }
            });
        ...
        function sumit(finalobject)
        {
            $.ajax({
                url: '<?=$api_base_url?>php-records/death',
                type: "POST",
                data: finalobject,
                dataType: 'json',
                success: function (data) {
                ...
                error: function () {
                    errorMsg("<?php echo $this->lang->line('server_error') ?>");
                    $("#formaddbtn").button('reset');
                }
            });
        }
    ...
    $(document).ready(function (e) {
        ...
        var fileInput = document.getElementById('edocument').files[0];
        var documentUrl = $('#filedataalread').val();

        let finalobject = {
            "patient_id": parseInt(formObject.epatient),
            "case_reference_id": parseInt(formObject.case_id),
            "attachment": "",
            "attachment_name": "",
            "death_date": formatDate(formObject.death_date),
        ...
        if (fileInput) {
            ...
            finalobject.attachment = $('#oldfile').val();
            finalobject.attachment_name =  $('#oldname').val();
            // console.log(JSON.stringify(finalobject, null, 2));
            sumit(finalobject)
            $("#formaddbtn").button('reset');
        }
    ...
    function sumit(finalobject) {
        var id = $('#eid').val();
        $.ajax({
            url: '<?=$api_base_url?>php-records/death/' + id,
            type: "PATCH",
            data: finalobject,
            dataType: 'json',
            success: function (data) {
    ...
    function getRecord(id) {
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                $("#oldfile").val(data.attachment);
                $("#oldname").val(data.attachment_name);

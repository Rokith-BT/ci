...
<script>
    $('#certificateform').submit(async function (e) {
        ...
        let backgroundInput = $('#documents').prop('files')[0];
        if (!backgroundInput) {
            finalData.background = '<?php echo $editidcard[0]->background; ?>';
        } else {
            finalData.background = await uploadImage(backgroundInput);
        }

        let logoInput = $('#logo_img').prop('files')[0];
        if (!logoInput) {
            finalData.logo = '<?php echo $editidcard[0]->logo; ?>';
        } else {
            finalData.logo = await uploadImage(logoInput);
        }

        let signInput = $('#sign_image').prop('files')[0];
        if (!signInput) {
            finalData.sign_image = '<?php echo $editidcard[0]->sign_image; ?>';
        } else {
            finalData.sign_image = await uploadImage(signInput);
        }

        $.ajax({
            url: '<?= $api_base_url ?>patient-id-card/' + <?= $editidcard[0]->id ?>,
            type: 'PATCH',

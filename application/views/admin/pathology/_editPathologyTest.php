...
?>
<input type="hidden" name="id" value="<?php echo $result->id;?>" id='editid'>
<div class="row">
                ...
                <tbody>
                    ...
                    if(!empty($result->pathology_parameter)){
                        foreach ($result->pathology_parameter as $patho_param_key => $patho_param_value) {
                         ...
                         ?>
                         <input type="hidden" name="prev_inserted[]" value="<?php echo $patho_param_value->id;?>" id='prev_inserted_<?=$count_id?>'>
                    <tr id="row<?php echo $row_count; ?>">
                        ...
                        <td>
                            ...
                            <button type="button" onclick="deletetestparameter(<?php echo $patho_param_value->id;?>)" class="closebtn delete_row" autocomplete="off"><i class="fa fa-remove"></i></button>
...
<script>
   function deletetestparameter(id) {
       $.ajax({
           type: "DELETE",
           url: "<?=$api_base_url?>pathology-test/removePathologyparameterdetails/" + id + "?hospital_id=<?=$data['hospital_id']?>",
           success: function (response) {
               console.log('deleted');

               // Find the row associated with the deleted parameter and remove it
               $("#row" + id).remove();

               // Optionally, show a success message
               successMsg('Test parameter deleted successfully');
           },
           error: function(xhr, status, error) {
               console.error('Error occurred while deleting the test parameter:', error);
               alert('Failed to delete the test parameter.');
           }
       });
   }
</script>

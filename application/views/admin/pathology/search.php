                ...
                <div class="box box-primary">
                    <div class="box-header with-border">
                        ...
                        <div class="box-tools pull-right">
                            ...
                            <button class="btn btn-primary btn-sm pathology addtest"
                                data-loading-text="<i class='fa fa-circle-o-notch fa-spin'></i> Please wait.."><i
                                    class="fa fa-plus"></i>
                                <?php echo $this->lang->line('add_pathology_test'); ?></button>
                            <?php } ?>
                    ...
                    <div class="box-body">
                        ...
                        <div class="table-responsive">
                            <table class="table table-striped table-bordered table-hover ajaxlist" cellspacing="0"
                                width="100%" data-export-title="<?php echo $this->lang->line('pathology_test'); ?>">
                                <thead>
                                    <tr class="white-space-nowrap">
                                        ...
                                        <th class="text-right">
                                            <?php echo $this->lang->line('charge') . " (" . $currency_symbol . ")"; ?>
                                        </th>
                                        <th class="text-right">
            ...
            <div class="modal-header modal-media-header">
                ...
                <div class="modalicon">
                    <div id='edit_delete'>
                        ...
                        <a href="#" data-target="#editModal" data-toggle="modal" title=""
            ...
            success: function(data) {
                if (data.status == "fail") {
                    ...
                    if (click_btn_id == "billsave") {
                        ...
                        $('#description,#report_date,#charge_category_html,#code_html,#charge_html,#apply_charge')
                            .val("");
                        //=============================
...
$(document).on('click', '.add-record', function() {
    ...
    var div = "<td width='35%'><input type='hidden' name='total_rows[]' value='" + pathology_parmeter_rows +
        "'>  <input type='hidden' name='inserted_id_" + pathology_parmeter_rows +
        "' value='0'>  <select class='form-control select2 pathology_parmeter' name='parameter_name_" +
        pathology_parmeter_rows +
        "' ><option value='<?php echo set_value('parameter_name'); ?>'><?php echo $this->lang->line('select'); ?></option><?php foreach ($parametername as $dkey => $dvalue) { ?><option value='<?php echo $dvalue["id"]; ?>'><?php echo $dvalue["parameter_name"] ?></option><?php } ?></select></td><td width='30%'><input type='text' name='reference_range_" +
        pathology_parmeter_rows + "' readonly id='reference_range" + pathology_parmeter_rows +
        "' class='form-control reference_range'></td><td width='30%'><input type='text' name='patho_unit_" +
        pathology_parmeter_rows + "' readonly id='patho_unit" + pathology_parmeter_rows +
        "' class='form-control patho_unit'></td>";

    var row = table.insertRow(table_len).outerHTML = "<tr id='row" + pathology_parmeter_rows + "'>" + div +
        "<td><button type='button'  class='closebtn delete_row'><i class='fa fa-remove'></i></button></td></tr>";
    $('.select2').select2();
    ...
    $.ajax({
        ...
        success: function(res) {
            if (res) {
                ...
                $('#amount').val((parseFloat((res.standard_charge) * res.percentage / 100) + (
                    parseFloat(res.standard_charge))).toFixed(2));
...
function viewDetail(id, value) {
    ...
    $.ajax({
        ...
        success: function(data) {
            ...
            $('#edit_delete').html(
                "<?php if ($this->rbac->hasPrivilege('pathology_test', 'can_edit')) { ?><a href='#' class='edittest' data-record-id='" +
                id +
                "' data-loading-text='<i class= \"fa fa-circle-o-notch fa-spin\"></i>' data-toggle='tooltip'  data-original-title='<?php echo $this->lang->line('edit'); ?>'><i class='fa fa-pencil'></i></a><?php }
                                                                                                                                                                                                                                                                                                                                                                if ($this->rbac->hasPrivilege('pathology_test', 'can_delete')) {
                                                                                                                                                                                                                                                                                                                                                                    ?><a onclick='delete_record(" +
                id +
...
$(document).on('click', '.edittest', function() {
    ...
    $.ajax({
        ...
        success: function(res) {
            ...
            var post_charge_category_id = $('#addTestReportModal .modal-body').find(
                "input[name='post_charge_category_id']").val();
            var post_charge_id = $('#addTestReportModal .modal-body').find(
                "input[name='post_charge_id']").val();
            $('.select2').select2();
...
function delete_record(id) {
    if (confirm('<?php echo $this->lang->line('delete_confirm') ?>')) {
        $.ajax({
            url: '<?= $api_base_url ?>pathology-test/' + id + '?hospital_id=<?= $data['hospital_id'] ?>',
            type: "DELETE",
            data: {
                id: id
            },
...
$(document).ready(function() {
    $("#formadd").on('submit', function(e) {
        ...
        for (var i = 1; i <= parseInt(formObject["total_rows[]"] || 0); i++) {
            if ($("#parameter_name_" + i).val() !== '') {
                requiredFields.push({
                    key: "parameter_name_" + i,
                    name: "Test Parameter Name",
                    message: 'is required'
                });
            }

            if ($("#reference_range_" + i).val() !== '') {
                requiredFields.push({
                    key: "reference_range_" + i,
        ...
        var missingFields = requiredFields.filter(field => !formObject[field.key]);

        if (missingFields.length > 0) {
            let message = "Please correct the following errors:\n" +
                missingFields.map(field => `• ${field.name}: ${field.message}`).join("\n");
        ...
        var totalRows = parseInt(formObject["total_rows[]"] || 0);
        var additionalData = [];

        for (var i = 1; i <= totalRows; i++) {
            var pathologyParameterId = formObject["parameter_name_" + i];
            var pathologyId = formObject["inserted_id_" + i];
            if (pathologyParameterId && pathologyId) {
                additionalData.push({
                    "pathology_id": parseInt(pathologyId),
                    "pathology_parameter_id": parseInt(pathologyParameterId),
                    "hospital_id": <?= $data['hospital_id'] ?>
                });
            }
        }

        var modifiedFormObject = {
            "test_name": formObject.test_name,
            "short_name": formObject.short_name,
            "test_type": formObject.test_type,
            "pathology_category_id": parseInt(formObject.pathology_category_id),
            "sub_category": formObject.sub_category,
            "report_days": formObject.report_days || "0",
            "method": formObject.method,
            "charge_id": parseInt(formObject.code),
            "hospital_id": <?= $data['hospital_id'] ?>,
            "unit": "none"
        };
        ...
        if (!editid) {
            api1 = '<?= $api_base_url ?>pathology-test';
            method = "POST";
            api2 = '<?= $api_base_url ?>pathology-test/pathologyParameterDetails';
        } else {
            api1 = '<?= $api_base_url ?>pathology-test/' + editid;
            method = "PATCH";
            api2 = '<?= $api_base_url ?>pathology-test/editpathoparameterdetail/parameter';
        }

        let previousIds = $('[id^="prev_inserted_"]').map(function() {
            return $(this).val();
        }).get();
        ...
        $.ajax({
            url: api1,
            type: method,
            data: JSON.stringify(modifiedFormObject),
            dataType: 'json',
            contentType: 'application/json',
            success: function(response) {
                let pathologyId = response[0]?. ['data ']?.Pathology_Values?. [0]?.id ||
                    response[0]?. ['data ']?.updated_values?. [0]?.id || null;
                if (pathologyId) {
                    additionalData = additionalData.map(item => ({
                        "pathology_id": pathologyId,
                        "pathology_parameter_id": item.pathology_parameter_id,
                        "hospital_id": <?= $data['hospital_id'] ?>,
                        'id': item.id
                    }));
                    ...
                    $.ajax({
                        ...
                        success: function(response) {
                            let message = (response.data && Array.isArray(
                                        response.data) && response.data.length >
                                    0 && response.data[0].message) || response
                                .message || "An error occurred.";
                            successMsg(message);
                            window.location.reload(true);
                        },
                        error: function() {
                            errorMsg('Error occurred while saving data.');
                        },
                        complete: function() {

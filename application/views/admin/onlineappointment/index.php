        ...
        <div class="row">
            ...
            <div class="col-md-10">
                ...
                <div class="box box-primary">
                    ...
                    <form action="<?php echo site_url('admin/onlineappointment/') ?>" id="doctor_form" method="post"
                            ...
                            <div class="row">
                                <div class="col-lg-3 col-md-3">
                                    <div class="form-group">
                                        <label><?php echo $this->lang->line('doctor'); ?><small class="req">
                                                *</small></label>
                                        <select autofocus="" style="width: 100%" onchange="getDoctorShift()" id="doctor"
                                            name="doctor" class="select2">
                                            ...
                                            <option value="<?php echo $doctor['id'] ?>" <?php
                                                ...
                                                (<?php echo $doctor['employee_id'] ?>)</option>
                                            <?php } ?>
                                        </select>
                                ...
                                <div class="col-lg-3 col-md-3">
                                    <div class="form-group">
                                        <label><?php echo $this->lang->line("shift"); ?><small class="req">
                                                *</small></label>
                                ...
                                <div class="col-lg-3 col-md-3 mt-5">
                                    ...
                                    <div class="form-group">
                                        <button type="button" onclick="search()"
                                            class="btn btn-primary btn-sm"><?php echo $this->lang->line('search'); ?></button>
                                    </div>
                                </div>
                            </div>
                    </form>
                    ...
                    <div class="row">
                        <div class="col-md-4">
                            <div class="form-group">
                                <label><?php echo $this->lang->line("consultation_duration"); ?><small class="req">
                                        *</small></label>
                                <input type="number" name='consult_time' form=""
                                    value="<?php echo isset($duration) ? $duration : ''; ?>" placeholder=''
                                    class="form-control" id="consult_time" />
                            </div>
                        </div>
                        <div class="col-md-4">
                            <div class="form-group">
                                <label><?php echo $this->lang->line('charge_type'); ?></label>
                                <select name="charge_type" class="form-control charge_type select2" style="width: 100%">
                                    <option value="" disabled selected><?= $this->lang->line('select'); ?></option>
                                    <?php if (!empty($charge_type)) {
                                        foreach ($charge_type as $row) {
                                            if ($row->is_active === 'yes') { 
                                                $selected = isset($charge_type_id) && $charge_type_id == $row->id ? "selected" : "";
                                                ?>
                                                <option value="<?= $row->id ?>" <?= $selected ?>>
                                                    <?= htmlspecialchars($row->charge_type) ?>
                                                </option>
                                    <?php   }
                                        }
                                    } ?>
                        ...
                        <div class="col-md-4">
                            <div class="form-group">
                                ...
                                <select name="charge_category" style="width: 100%" form=""
                                    ...
                                    <option value="" disabled selected><?php echo $this->lang->line('select'); ?>
                                    </option>
                                    <?php foreach ($charge_category as $key => $value) {
                                        ?>
                                    <option
                                        <?php echo isset($charge_category_id) ? $charge_category_id == $value['id'] ? "selected" : "" : ''; ?>
                        ...
                        <div class="col-md-6">
                            <div class="form-group">
                                ...
                                <select name="charge_id[]" id="charge_id" style="width: 100%" form=""
                                    ...
                                    <option
                                        <?php echo isset($charge_id) && in_array($charge_value['id'], (array)$charge_id) ? "selected" : ""; ?>
                                        value="<?php echo $charge_value['id']; ?>">
                        ...
                        <div class="col-md-6">
                            <div class="form-group">
                                ...
                                <input type="text" class="form-control standard_charge"
                                    value="<?php echo isset($appointment_charge) ? amountFormat($appointment_charge) : ''; ?>"
                                    disabled="disabled" />
                            </div>
                        </div>
                    </div>
                ...
                <div class="nav-tabs-custom border0">
                    <ul class="nav nav-tabs" id="myTabs">
                        ...
                        <li <?php echo $cls; ?>><a href="#tab_<?php echo $count; ?>"
                                data-s="<?php echo set_value("shift") ?>" data-d="<?php echo set_value("doctor"); ?>"
                                data-day="<?php echo $days_key; ?>" data-toggle="tab"
                                aria-expanded="true"><?php echo $days_value; ?></a></li>
                        <?php $count++;
                                } ?>
                    </ul>
...
$(document).ready(function() {
    ...
    $(document).on("click", ".addrow", function() {
        var newRow = $("<tr>");
        var cols = "";
        cols += '<td><input type="hidden" name="total_row[]" value="' + tot_count +
            '"><input type="hidden" name="prev_id_' + tot_count +
            '" value="0"><div class="input-group"><input type="text" name="time_from_' + tot_count +
            '" class="form-control time_from time" id="time_from_' + tot_count +
            '"  aria-invalid="false"><div class="input-group-addon"><i class="fa fa-clock-o"></i></div></div></td>';

        cols += '<td><div class="input-group"><input type="text" name="time_to_' + tot_count +
            '" class="form-control time_to time" id="time_to_' + tot_count +
            '"  aria-invalid="false"><div class="input-group-addon"><i class="fa fa-clock-o"></i></div></div></td>';

        cols +=
            '<td width="30" class="text-right"><button type="button" class="ibtnDel btn btn-danger btn-sm btn-danger"><i class="fa fa-trash"></i></button></td>';
        newRow.append(cols);
...
$(document).on('select2:select', '.charge_type', function () {
    ...
    $('.charge').html("<option value='' disabled selected>Select</option>").val('').trigger("change");
    getcharge_category(charge_type, "");
});
function getcharge_category(charge_type) {
    var div_data = "<option value='' disabled selected>Select</option>";
    if (charge_type !== "") {
        $.ajax({
            ...
            success: function (res) {
                $.each(res, function (i, obj) {
                    var sel = (obj.id == charge_id) ? " selected" : "";
                    div_data += "<option value='" + obj.id + "'" + sel + ">" + obj.name + "</option>";
                });
                $('.charge_category').html(div_data).val(charge_id).trigger("change");
            }
        });
    } else {
        $('.charge_category').html("<option value='' disabled selected>Select</option>").val('').trigger("change");
        $('.charge').html("<option value='' disabled selected>Select</option>").val('').trigger("change");
    }
}
    ...
    if (charge_category !== "") {
        $.ajax({
            ...
            success: function (res) {
                ...
                $('.charge').html(div_data).val(charge_id).trigger("change");
            }
        });
    } else {
        $('.charge').html("<option value='' disabled selected>Select</option>").val('').trigger("change");
...
$(document).on('select2:select', '.charge', function () {
    ...
    if (charge !== "") {
        $.ajax({
            ...
            dataType: 'json',
            success: function (res) {
                let standard_charge = parseFloat(res.standard_charge) + (parseFloat(res.standard_charge) * parseFloat(res.percentage) / 100);

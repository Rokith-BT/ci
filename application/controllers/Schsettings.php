...
class Schsettings extends Admin_Controller
{
    ...
    public function index()
    {
        ...
        $data['languagelist'] = $this->language_model->getEnable_languages();
        $data['timezoneList'] = $this->customlib->timezone_list();
        $data['timeFormat'] = $this->customlib->timeFormat();
        $data['monthList'] = $this->customlib->getMonthList();
        $data['dateFormatList'] = $this->customlib->getDateFormat();
        $data['currencyList'] = $this->customlib->getCurrency();

        $hospital_data = $this->session->userdata('hospitaladmin');
        $token = $hospital_data['accessToken'] ?? '';
        $api_url = "{$api_base_url}settings-general-setting/{$hospital_data['hospital_id']}";

        $makeRequest = function ($accessToken) use ($api_url) {
            $ch = curl_init();
        ...
        if (isset($responseData['status_code']) && $responseData['status_code'] == 403) {
            ...
            $hospital_data['accessToken'] = $newToken;
            $this->session->set_userdata('hospitaladmin', $hospital_data);
            $response = $makeRequest($newToken);
            $responseData = json_decode($response, true);
        }

        $data['api_data'] = $responseData['data'] ?? ['error' => 'Failed to retrieve API data'];
    ...
    public function departmentqr()
    {
        ...
        $data['initialData'] = $this->customlib->getpagenationfull('consultation-process-list');
        $this->load->view('layout/header', $data);
        $this->load->view('setting/departmentqr', $data);
        $this->load->view('layout/footer', $data);
    }

     public function consultationprocess()
    {
        $id = $this->input->post('id');
        $hospital_data = $this->session->userdata('hospitaladmin');
        $api_url = "consultation-process-list/QR/{$id}?hospital_id={$hospital_data['hospital_id']}";
        $data = $this->customlib->getpagenationfull($api_url);
        echo json_encode( $data);
    }
}

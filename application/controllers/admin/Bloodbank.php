...
class Bloodbank extends Admin_Controller
{
    public function __construct()
    {
        ...
        if (!$this->Admin_model->validationModule('blood_bank')) {
            access_denied();
        }
    }
        ...
        if (!empty($dt_response->data)) {
            foreach ($dt_response->data as $key => $value) {
                ...
                if ($this->rbac->hasPrivilege('blood_bank_product', 'can_delete')) {
                    $action .= "<a class='btn btn-default btn-xs' data-toggle='tooltip' title='' onclick='delete_recordById(\"" . $api_base_url . "setup-blood-bank-products/" . $value->id . "?Hospital_id=" . $data['hospital_id'] . "\")' data-original-title='" . $this->lang->line('delete') . "'> <i class='fa fa-trash'></i></a>";

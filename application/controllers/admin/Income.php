...
class Income extends Admin_Controller
{
    ...
    public function __construct()
    {
        ...
        $this->load->helper(['form', 'customfield_helper']);
        $this->load->library("datatables");
        $this->load->model(["transaction_model", "Admin_model"]);
        ...
        if (!$this->Admin_model->validationModule('income')) {
            access_denied();
        }
    }
    ...
    public function index()
    {
        ...
        $data['initialData'] = $this->customlib->getpagenation('finance-income/v2/getfinanceincome');
    ...
    public function getDatatable()
    {
        ...
        if (!empty($dt_response->data)) {
            foreach ($dt_response->data as $value) {
                $row = array();
                $column_first = '<a href="#" data-toggle="popover" class="detail_popover">' . htmlspecialchars($value->name) . '</a>';
                ...
                if (!empty($value->documents)) {
                    ...
                    $column_first .= "<a href='data:" . $mimeType . ";base64," . $base64File . "' download='" . htmlspecialchars($fileInfo['basename']) . "' class='btn btn-default btn-xs' data-toggle='tooltip' title='" . $this->lang->line('download') . "'><i class='fa fa-download'></i></a>";
                }


                if ($this->rbac->hasPrivilege('income', 'can_edit')) {
                    $column_first .= ' <a onclick="edit(' . intval($value->id) . ')" class="btn btn-default btn-xs" data-toggle="tooltip" title="' . $this->lang->line('edit') . '"><i class="fa fa-pencil"></i></a>';
                }

                if ($this->rbac->hasPrivilege('income', 'can_delete')) {
                    $column_first .= ' <a class="btn btn-default btn-xs" data-toggle="tooltip" title="' . $this->lang->line('delete') . '" onclick="delete_record(' . intval($value->id) . ')"><i class="fa fa-trash"></i></a>';
                }

                $column_first .= '</div>';
                $column_first .= '<div class="fee_detail_popover" style="display: none">';
                $column_first .= empty($value->note) ? '<p class="text text-danger">' . $this->lang->line('no_description') . '</p>' : '<p class="text text-info">' . htmlspecialchars($value->note) . '</p>';
                $column_first .= '</div>';

                $row[] = $column_first;
                $row[] = intval($value->invoice_no);
                $row[] = $this->customlib->YYYYMMDDTodateFormat($value->date);
                $row[] = htmlspecialchars($value->note);
                $row[] = htmlspecialchars($value->income_category);

                if (!empty($fields)) {
                    foreach ($fields as $fields_value) {
                        $display_field = $value->{$fields_value->name} ?? '';
                        if ($fields_value->type == "link") {
                            $display_field = "<a href=\"" . htmlspecialchars($display_field) . "\" target='_blank'>" . htmlspecialchars($display_field) . "</a>";
                        }
                        $row[] = $display_field;
                    }
                }

                $row[] = floatval($value->amount);
                $dt_data[] = $row;
            }
        }
    ...
    public function alltransactionreport($value = '')
    {
        ...
        $this->session->set_userdata('sub_menu', 'admin/income/alltransactionreport');
        $data['title'] = 'title';
        $data['staffsearch'] = $this->patient_model->getstaffsearch();
        $data["modules"] = $this->customlib->get_modules();
        $data["searchlist"] = $this->search_type;
        $data['search_data'] = '';
        $url = $api_base_url . "php-transaction-report/collected_by";
        $ch = curl_init();
        ...
        $data['collected_by_list'] = isset($decoded['statusCode']) && $decoded['statusCode'] == 404 ? [] : (is_array($decoded) ? $decoded : []);
        $this->load->view('layout/header', $data);
        $this->load->view('admin/income/alltransactionReport', $data);
        $this->load->view('layout/footer', $data);
    }
    ...
    public function getincomelist()
    {
        ...
        $url = $api_base_url . "finance-income/v2/get_finance_income?search=" . urlencode($search) . "&limit=$limit&page=$page";
    ...
    public function incomeReport()
    {
        ...
        if ($filtertype == 'total' || $filtertype == '') {
            ...
            $url = $api_base_url . 'php-income-report/total?page=' . $page . '&search=' . urlencode($search) . '&limit=' . $limit;
            ...
            $url = $api_base_url . 'php-income-report/month-wise?page=' . $page . '&search=' . urlencode($search) . '&limit=' . $limit;
            $makeRequest = function ($accessToken) use ($url, $data) {
                $ch = curl_init();
                curl_setopt($ch, CURLOPT_URL, $url);
                curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
            ...
            $response = $makeRequest($token);
            $responseData = json_decode($response, true);
        } else if ($filtertype == 'consultant') {

            $url = $api_base_url . 'php-income-report/doctor-wise?page=' . $page . '&search=' . urlencode($search) . '&limit=' . $limit;
            $makeRequest = function ($accessToken) use ($url, $data) {
                $ch = curl_init();
                curl_setopt($ch, CURLOPT_URL, $url);
                curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
            ...
            $url = $api_base_url . 'php-income-report/patient-wise?page=' . $page . '&search=' . urlencode($search) . '&limit=' . $limit;
            $makeRequest = function ($accessToken) use ($url, $data) {
                $ch = curl_init();
                curl_setopt($ch, CURLOPT_URL, $url);
                curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
        ...
        $output = [
            ...
            "recordsFiltered" => isset($responseData['total']) ? (int)$responseData['total'] : 0,
            "data" => isset($responseData['data']) ? $responseData['data'] : [],
            "payment_summary" => isset($responseData['header']) ? $responseData['header'] : [],
        ];
        echo json_encode($output);
    }
    ...
    public function Collection()
    {
        ...
        $this->session->set_userdata('top_menu', 'Reports');
        $this->session->set_userdata('sub_menu', 'admin/income/Collection');
        ...
        $data['collected_by_list'] = isset($decoded['statusCode']) && $decoded['statusCode'] == 404 ? [] : (is_array($decoded) ? $decoded : []);
        $this->load->view('layout/header', $data);
        $this->load->view('admin/income/Collection', $data);
        $this->load->view('layout/footer', $data);
    }


    public function Collection_generate_bill()
    {
        $data['hospital_name'] = "Plenome Hospital";
        $data['bill_title'] = "Collection Report - Cash Transactions";
        $data['collected_by'] = "UMA MAHESHWARI";
        $data['payment_type'] = "Cash";
        $report_date = date("d/m/Y");
        $generated_time = date("H:i:s");
        $combined = $report_date . ' ' . $generated_time;
        $formatted = date("F d, Y h:i:s A", strtotime(str_replace('/', '-', $combined)));
        $data['report_date'] = $report_date;
        $data['generated_time'] = $generated_time;
        $data['formatted_datetime'] = $formatted;
        $data['transactions'] = [
            [
                'bill_date' => '29/04/2015',
                'bill_no' => 'IPDN58',
                'link' => base_url() . 'admin/patient/ipdprofile/58#charges',
                'patient_id' => 'MHCA 67918',
                'patient_name' => 'Mrs. Elizabeth',
                'consultant' => 'Dr M.SUGANYA, M.D,DM (Nephrologist)',
                'amount' => 79920.00,
                'bill_type' => 'IP Income',
            ],
            [
                'bill_date' => '30/04/2015',
                'bill_no' => 'OPDN1902',
                'link' => base_url() . 'admin/patient/visitdetails/Mzcy/MTkwMg==#charges',
                'patient_id' => 'MHCA 67919',
                'patient_name' => 'Mr. John Doe',
                'consultant' => 'Dr A.KUMAR, MBBS (General)',
                'amount' => 1500.00,
                'bill_type' => 'OP Income',
            ],
        ];
        $data["print_details"] = $this->printing_model->get('', 'paymentreceipt');
        $data['total_transactions'] = count($data['transactions']);
        $data['total_amount'] = array_sum(array_column($data['transactions'], 'amount'));
        $page = $this->load->view('admin/income/Pdfcollection', $data);
    ...
    public function getallcollectionreport()
    {
        $date_from = $this->input->post('date_from');
        $date_to = $this->input->post('date_to');
        $search_value = $this->input->post('collect_staff');
        $search_type = $this->input->post('search_type');
        ...
        $hospitaldata = $this->session->userdata('hospitaladmin');
        $token = $hospitaldata['accessToken'] ?? '';
        $url = $api_base_url . 'php-collection-by-report?search=' . urlencode($search_value) . '&limit=' . $limit . '&page=' . $page;
    ...
    {
        $search_value = $this->input->post('patientname');
        $api_base_url = $this->config->item('api_base_url');
        $hospitaldata = $this->session->userdata('hospitaladmin');
        $token = $hospitaldata['accessToken'] ?? '';
        $url = $api_base_url . 'php-collection-by-report?name=' . urlencode($search_value);
    ...
    {
        $data['inclist'] = $this->customlib->getpagenation('finance-income/v2/getfinanceincome');
        echo json_encode($data['inclist']);
    }
}

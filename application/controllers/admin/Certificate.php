        ...
        if (!empty($dt_response->data)) {
            foreach ($dt_response->data as $key => $value) {
                ...
                if ($value->background_image != '' && !is_null($value->background_image)) {
                    ...
                    curl_setopt($client, CURLOPT_POSTFIELDS, json_encode(['value' => $value->background_image]));
                    ...
                    if ($response !== false) {
                        ...
                        $image = "<img src='" . $base64Image . "' width='40'>";
                    } else {
                ...
                if ($this->rbac->hasPrivilege('certificate', 'can_delete')) {
                    $action .= "<button type='button' class='btn btn-default btn-xs' data-toggle='tooltip' title='" . $this->lang->line('delete') . "' onclick='deleteCertificate(" . $value->id . ")'><i class='fa fa-remove'></i></button>";

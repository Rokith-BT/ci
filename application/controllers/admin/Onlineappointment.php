...
class Onlineappointment extends Admin_Controller
{
    public function __construct()
    {
        ...
        $this->load->model(array("onlineappointment_model", "charge_category_model","Chargetype_model"));
    ...
    public function index()
    {
        ...
        $data["charge_type"]     =$this->Chargetype_model->getChargeTypeByModule("appointment");
        ...
        if ($this->form_validation->run() == false) {
            ...
            $charge_type_id             =$this->Chargetype_model->getChargeTypeByModuleid($data['charge_id']);
            $data["charge_type_id"]     = isset($charge_type_id->charge_type_id) ? $charge_type_id->charge_type_id : '';
    ...
    public function getShiftdata()
    {
        ...
        $data["global_shift"] = $this->onlineappointment_model->getGlobalShift($this->input->post("shift"));
        $data["doctordetials"]      = $this->staff_model->get($doctor_id);
    ...
    public function getPatientSchedule()
    {
        ...
        if (!empty($dt_response->data)) {
            foreach ($dt_response->data as $key => $value) {
                ...
                $row = array();
                //====================================
                $column_first = '<a href="#" data-toggle="popover" class="detail_popover" onclick="getpatientData(' . $value->id . ')">' . $value->patient_name . " (" . $value->id . ") " . '</a>';

                //==============================

                $row[] = !empty($column_first) ? $column_first : "-";
                $row[] = !empty($value->mobileno) ? $value->mobileno : "-";
                $row[] = !empty($value->time) ? date("h:i A", strtotime($value->time)) : "Offline";
                $row[] = !empty($value->email) ? $value->email : "-";
                $row[] = !empty($value->date) ? date("d/m/Y", strtotime($value->date)) : "-";
                $row[] = !empty($value->source) ? $value->source : "-";
                $dt_data[] = $row;
            }
        }
        $json_data = array(
    ...
    public function globalShift()
    {
        ...
        $data["shift"] = $shift;
        $data['initialData'] = $this->customlib->getpagenation('setup-appointment-shift/v2/getAllshift');
        $this->load->view('layout/header');
    ...
    public function doctorGlobalShift()
    {
        ...
        $data['initialData'] = $this->customlib->getpagenation('setup-appointment-doctor-shift/v2/getAllpage');
        $this->load->view('layout/header');
    ...
    {
        $data['chargeslist'] = $this->customlib->getpagenation('setup-appointment-doctor-shift/v2/getAllpage');
    ...
    public function getShift()
    {
        ...
        $date = DateTime::createFromFormat('d/m/Y', $dates)->format('Y-m-d');
        $doctor       = $this->input->post("doctor");
        $global_shift = $this->input->post("global_shift");
        $day          = date("l", strtotime($date));
        $shift        = $this->onlineappointment_model->getShiftdataappointment($doctor, $day, $global_shift,$dates);
        echo json_encode($shift);
    }
    ...
    {
        $data['shiftlist'] = $this->customlib->getpagenation('setup-appointment-shift/v2/getAllshift');
        echo json_encode( $data['shiftlist']);
    }

    public function docglobalshift()
    {
        ...
        $url = $api_base_url . "setup-appointment-doctor-shift/v2/getAllpage?limit=$limit&page=$page&search=$search";

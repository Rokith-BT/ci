...
class Taxcategory extends Admin_Controller
{
    ...
    public function __construct()
    {
        ...
        public function index()
        {
            ...
            $data['initialData'] = $this->customlib->getpagenation('setup-hospital-charges-tax-category/v2/getALLtaxcategory');

            $this->config->load('payroll');

            $this->load->view('layout/header', $data);
            $this->load->view('admin/charges/taxCategory', $data);
            $this->load->view('layout/footer');
        }

        public function taxcategory()
    {
        $data['chargeslist'] = $this->customlib->getpagenation('setup-hospital-charges-tax-category/v2/getALLtaxcategory');
        ...
        if (!empty($dt_response->data)) {
            foreach ($dt_response->data as $tax_key => $tax_value) {
                ...
                if ($this->rbac->hasPrivilege('tax_category', 'can_delete')) {
                    $action .= "<a class='btn btn-default btn-xs' data-toggle='tooltip' title='' onclick='delete_recordById(\"" . $api_base_url . "setup-hospital-charges-tax-category/" . $tax_value->id . "?Hospital_id=" . $data['hospital_id'] . "\")' data-original-title='" . $this->lang->line('delete') . "'> <i class='fa fa-trash'></i></a>";

...
class Payroll extends Admin_Controller
{
    ...
    public function create($month, $year, $id)
    {
        ...
        $data["leave_count"]     = 0;
        $data["alloted_leave"]   = 0;
        $data["payroll_settings"] = $this->payroll_model->getpayroll();
        $user_type               = $this->staff_model->getStaffRole();
        $data["this_month_leaves"]  = $this->staff_model->get_leaves($month, $year, $id);
        $data['classlist']       = $user_type;
        $date                    = $year . "-" . $month;
    ...
    public function payrollNotification($staff_id, $role, $month, $amount, $staffname, $url)
    {
        ...
        if (!empty($staff_id)) {
            ...
            $notification_data = array(
                'notification_title' => $this->lang->line('notification_salary_paid'),
    ...
    public function payslipView()
    {
        ...
        if (!empty($result)) {
            $data["allowance"]          = $this->payroll_model->getAllowance($result["id"]);
            $data["positive_allowance"] = $this->payroll_model->getAllowance($result["id"], "positive");
            $data["negative_allowance"] = $this->payroll_model->getAllowance($result["id"], "negative");
            $data["result"]             = $result;

            $staff_leave_requests = $this->staff_model->get_leaves($result["month"], $result["year"], $result["staff_id"]);
            $staff_attendance     = $this->staff_model->staff_attendance($result["month"], $result["year"], $result["staff_id"]);

            $data["total_lop"] = $staff_attendance - $staff_leave_requests;

            if ($condition == 'download') {
                $page = $this->load->view("admin/payroll/payslipview", $data, true);
                echo json_encode([
                    'status' => 'success',
                    'page'   => $page
                ]);
    ...
    public function payrollreports()
    {
        ...
        if (!empty($reportdata->data)) {
            foreach ($reportdata->data as $key => $value) {
                ...
                $total_gross += ($value->total_allowance - $value->total_deduction);
                $total_tax += $value->tax;
                $gross_salery = $value->total_allowance - $value->total_deduction;
                ...
                $row[]     = (number_format($value->total_allowance, 2, '.', ''));
    ...
    public function getpayrollreport()
    {
        ...
        $url = $api_base_url . 'php-payroll-report?page=' . $page . '&search=' . $search_value . '&limit=' . $limit;

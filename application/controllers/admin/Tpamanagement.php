...
class Tpamanagement extends Admin_Controller
{
    ...
    public function __construct()
    {
        ...
        if (!$this->Admin_model->validationModule('tpa_management')) {
            access_denied();
        }
    }
        ...
        if (!empty($dt_response->data)) {
            foreach ($dt_response->data as $key => $value) {
                ...
                if ($this->rbac->hasPrivilege('organisation', 'can_delete')) {
                    $action .= "<a href='#' onclick=delete_tpa($value->id) class='btn btn-default btn-xs pull-right'  data-toggle='tooltip' title='' data-target='#editmyModal'  data-original-title=" . $this->lang->line('delete') . "><i class='fa fa-trash'></i></a>";
                }
                $action .= "</div>";
                //==============================

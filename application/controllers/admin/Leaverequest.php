...
class Leaverequest extends Admin_Controller
{
    ...
    public function getapproveleaverequest()
    {
        $data['approveleaverequest'] = $this->customlib->getpagenation('human-resource-apply-leave/v2/get_staff_leave_request_details');
        echo json_encode($data['approveleaverequest']);
    }
    ...
    public function countLeave($id)
    {
        ...
        if (!empty($alloted_leavetype)) {
            $data = [];
            foreach ($alloted_leavetype as $i => $value) {
                $count_leaves = $this->leaverequest_model->countLeavesData($id, $value["leave_type_id"]);
    
                $data[$i] = [
                    'type'          => $value["type"],
                    'id'            => $value["leave_type_id"],
                    'alloted_leave' => $value["alloted_leave"] ?? 0,
                    'approve_leave' => $count_leaves['approve_leave'] ?? 0
                ];
            }
    
            $hasAvailableLeave = false;
    
            foreach ($data as $dvalue) {
                $selected = ($lid == $dvalue["id"]) ? "selected" : "";
                $available = max((int)$dvalue["alloted_leave"] - (int)$dvalue["approve_leave"], 0);    
                if ($available > 0) {
                    $html .= "<option value='" . $dvalue["id"] . "' $selected>" . $dvalue["type"] . " (" . $available . ")" . "</option>";
                    $hasAvailableLeave = true;
                }
            }
    
            if (!$hasAvailableLeave) {
                $html .= $this->getDefaultLeaveOptions();
            }
        } else {
            $html .= $this->getDefaultLeaveOptions();
        }
    ...
    private function getDefaultLeaveOptions()
    {
        $results = $this->leaverequest_model->getLeaveDetails();
        $options = "";
    
        if (!empty($results)) {
            foreach ($results as $result) {
                $options .= "<option value='" . $result["id"] . "'>" . $result["type"] . "</option>";
            }
        }
    ...
    public function remainingleave($id, $leavetypeid)
    {
        $lop = $this->leaverequest_model->getLeaveDetails();
        $isLopType = false;
    
        if (!empty($lop)) {
            foreach ($lop as $leave) {
        ...
        if (!$isLopType) {
            $staff_leave = $this->leaverequest_model->myallotedLeaveType($id, $leavetypeid);
            $approve_leave = $this->leaverequest_model->countLeavesData($id, $leavetypeid);
    
            $alloted_leave = isset($staff_leave['alloted_leave']) ? (int)$staff_leave['alloted_leave'] : 0;
            $approved_leave = isset($approve_leave['approve_leave']) ? (int)$approve_leave['approve_leave'] : 0;
    
            $pending_leave = max(0, $alloted_leave - $approved_leave);
    
            echo json_encode(["leaveCount" => $pending_leave]);
        } else {
            echo json_encode(["leaveCount" => 100000000]);
        }
    }

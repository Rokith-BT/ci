...
class Bill extends Admin_Controller
{
    ...
    public function __construct()
    {
        ...
        $this->load->model(array(
            'charge_model',
            'patient_model',
            'appoint_priority_model',
            'onlineappointment_model',
            'transaction_model',
            'conference_model',
            'casereference_model',
            'Admin_model'
        ...
        if (!$this->Admin_model->validationModule('bill')) {
            access_denied();
        }
    }
    ...
    public function dashboard()
{
    ...
    if ($search_type && $search_value) {
        $this->form_validation->set_rules('search_value', $this->lang->line($search_type), 'required|trim|xss_clean|numeric');
        if ($this->form_validation->run() == true) {
            ...
            switch ($search_type) {
                case 'opd_id':
                    $url = $api_base_url . "billing/v2/get_billing_details_by_opdID?opd_id=$search_value&search=&limit=10&page=1";
                    $ch = curl_init();
                    ...
                    break;
                case 'ipd_id':
                    $url = $api_base_url . "billing/v2/get_billing_details_by_opdID?opd_id=$opd_id&search=$search&limit=$limit&page=$page";
                    $ch = curl_init();
                    curl_setopt($ch, CURLOPT_URL, $url);
                    curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
                    curl_setopt($ch, CURLOPT_HTTPHEADER, [
                ...
                case 'case_id':
                    $patient = $this->patient_model->getDetailsByCaseId($search_value);
                    $query_param = "case_id";
                    break;

                case 'patient_id':
                    $patient = $this->patient_model->getDetailsByPatientId($search_value, $search_type);
                    $query_param = "patient_id";
                    break;
            }
            if ($responseData["total"]!=0) {
                redirect('admin/bill/index/' . base64_encode($search_value) . '?case=' . $query_param);
            } else {
                $data['error_message'] = $this->lang->line('no_record_found');
            }

        } else {
            $data['error_message'] = strip_tags(validation_errors());
        }
    }

    $data['search_type'] = $search_type ?? null;
    $data['search_value'] = $search_value ?? null;
    ...
    {
        $search = $_GET['case'] ?? NULL;
        $charges                = $this->charge_model->getopdChargesbyCaseId($case_id, $search);
    ...
    {
        $search = $_GET['case'] ?? NULL;
        $charges                = $this->charge_model->getipdChargesbyCaseId($case_id, $search);
        $data["charges_detail"] = $charges;
        $data["ipdid"] = $case_id;
        $page                   = $this->load->view('admin/bill/_ipd_charges', $data, true);
        echo json_encode(array('status' => 1, 'page' => $page));
    }
    public function getipddetials($case_id)
    {
        $search = $_GET['case'] ?? NULL;
        $charges                = $this->charge_model->getipdChargesbyCaseId($case_id, $search);
        $data["charges_detail"] = $charges;
    ...
    public function getDetailsByCaseId($case_id)
    {
        ...
        if ($search == "patient_id") {
            $patient = $this->patient_model->getDetailsByPatientId($case_id, $search);
        } else if ($search == "opd_id") {
            $api_base_url = $this->config->item('api_base_url');
            $url = $api_base_url . "billing/v2/get_billing_details_by_opdID?opd_id=$case_id&search=&limit=10&page=1";
            $response = file_get_contents($url);
            $data = json_decode($response, true);
            $patient = $data['patientInfo'] ?? [];
        } else if ($search == "ipd_id") {
            $api_base_url = $this->config->item('api_base_url');
            $url = $api_base_url . "billing/v2/get_billing_details_by_ipdID?ipd_id=$case_id&search=&limit=10&page=1";
            $response = file_get_contents($url);
            $data = json_decode($response, true);
            $patient = $data['patientInfo'] ?? [];
        } else {
    ...
    public function getpayment()
    {
        ...
        $payment_details         = $this->transaction_model->getPatientPaymentsByCaseId($case_id, $module_type, $id, $search, $patient_id);
    ...
    public function patient_bill()
    {
        ...
        $search                   = $this->input->post('search');
        if ($search == "patient_id") {
            $data['opd_data']         = $this->charge_model->getopdChargesbyCaseId($case_id, $search);
            $data['ipd_data']         = $this->charge_model->getipdChargesbyCaseId($case_id, $search);
        } else if (($search == "opd_id")) {
            $data['opd_data']         = $this->charge_model->getopdChargesbyCaseId($case_id, $search);
        } else if (($search == "ipd_id")) {
            $data['ipd_data']         = $this->charge_model->getipdChargesbyCaseId($case_id, $search);
        } else if ($search == "case_id") {
            $data['opd_data']         = $this->charge_model->getopdChargesbyCaseId($case_id, $search);
            $data['ipd_data']         = $this->charge_model->getipdChargesbyCaseId($case_id, $search);
        }
        ...
        $data['transaction_data'] = $this->transaction_model->getTransactionByCaseId($case_id, $search);
        $data['refund_data'] = $this->transaction_model->getRefundByCaseId($case_id);
        $page                     = $this->load->view('admin/bill/_patient_bill', $data, true);
        $modal_action             = "<a href='javascript:void(0);' data-case-id=" . $case_id . " data-search-value=" . $search . " class='print_bill d-inline'><i class='fa fa-print'></i></a>";
    ...
    public function generate_bill()
    {
        ...
        $id                    = $this->input->post('id');
        $print_details         = $this->printing_model->get('', 'paymentreceipt');
        $patient               = $this->patient_model->getDetailsByCaseId($case_id, $search);
        $data['patient']       = $patient;
        $data['print_details'] = $print_details;
        $data['case_id']       = $case_id;
        $transaction           = $this->transaction_model->ipdopdPaymentByCaseId($case_id, $search);
        $data['paid_amount']   = $transaction;
        $refund                = $this->transaction_model->getopdIpdrefundbyCaseId($case_id, $search);
        if (!empty($refund)) {
        ...
        $data['charge_details'] = $this->transaction_model->get_ipdopdchargebycaseId($case_id, $search, $module_type);
        $data['module_type']    = $module_type;
    ...
    public function generate_bill_result()
    {
        ...
        $transaction            = $this->transaction_model->ipdopdPaymentByCaseId($case_id, $search);
        $data['paid_amount']    = $transaction;
        $data['charge_details'] = $this->transaction_model->get_ipdopdchargebycaseId($case_id, $search, $id, $patient_id);
    ...
    public function getopddatatable()
    {
        ...
        if (!empty($dt_response->data)) {
            foreach ($dt_response->data as $key => $value) {
                $row = array();
                //====================================
                $action = "<div class='rowoptionview rowview-mt-19 text-center'>";
                ...
                //============================== Aligning data center
                $row[] = "<div class='text-center'>" . $first_action . (!empty($value->patient_name) ? $value->patient_name : '-') . "</a>" . $action . "</div>";
                $row[] = "<div class='text-center'>" . (!empty($value->patientid) ? $value->patientid : '-') . "</div>";
                $row[] = "<div class='text-center'>" . (!empty($value->guardian_name) ? $value->guardian_name : '-') . "</div>";
                $row[] = "<div class='text-center'>" . (!empty($value->gender) ? $value->gender : '-') . "</div>";
                $row[] = "<div class='text-center'>" . (!empty($value->mobileno) ? $value->mobileno : '-') . "</div>";
                $row[] = "<div class='text-center'>" . (!empty($value->name) || !empty($value->surname) ? composeStaffNameByString($value->name, $value->surname, $value->employee_id) : '-') . "</div>";

                // Convert last visit date to d/m/Y format
                $row[] = "<div class='text-center'>" . (!empty($value->last_visit) ? date('d/m/Y', strtotime($value->last_visit)) : '-') . "</div>";

                //====================
                if (!empty($fields)) {
                    foreach ($fields as $fields_key => $fields_value) {
                        $display_field = !empty($value->{"$fields_value->name"}) ? $value->{"$fields_value->name"} : '-';
                ...
                //====================
                $row[]     = "<div class='text-center'>" . (!empty($value->total_visit) ? $value->total_visit : '-') . "</div>";
                $dt_data[] = $row;
            }
        }
    ...
    public function getvisitdatatable($opdid)
    {
        ...
        if (!empty($dt_response->data)) {
            foreach ($dt_response->data as $key => $value) {
                ...
                $row[] = "<div class='text-center'>" . ($this->customlib->YYYYMMDDHisTodateFormat($value->appointment_date, $this->time_format) ?: "-") . "</div>";
                $row[] = "<div class='text-center'>" . (composeStaffNameByString($value->name, $value->surname, $value->employee_id) ?: "-") . "</div>";
                $row[] = "<div class='text-center'>" . ($value->refference ?: "-") . "</div>";
                $row[] = "<div class='text-center'>" . (nl2br($value->symptoms) ?: "-") . "</div>";

                if (!empty($fields)) {
                    foreach ($fields as $fields_key => $fields_value) {
                        $display_field = $value->{"$fields_value->name"} ?: "-";
                        if ($fields_value->type == "link") {
                            $display_field = $value->{"$fields_value->name"} ? "<a href='" . $value->{"$fields_value->name"} . "' target='_blank'>" . $value->{"$fields_value->name"} . "</a>" : "-";
                        }
                        $row[] = "<div class='text-center'>" . $display_field . "</div>";
                    }
                }
    ...
    public function appointment()
    {
        ...
        $patients                      = $this->patient_model->getPatientListall();
        $data["patients"]              = $patients;
        $data["appointment_status"]    = $this->appoint_priority_model->Appointment_status();
        $data["yesno_condition"]       = $this->yesno_condition;
        $userdata                      = $this->customlib->getUserData();
        $role_id                       = $userdata['role_id'];
        $data["bloodgroup"]            = $this->bloodbankstatus_model->get_product(null, 1);
        ...
        if (!empty($dt_response->data)) {
            foreach ($dt_response->data as $key => $value) {
                ...
                if ($status == 'Reserved' || $status == 'Requested') {
                    $action .= "<a href='#' data-toggle='tooltip' title='" . $this->lang->line('additional_option') . "' class='btn btn-default btn-xs' onclick='additionalFunction(" . $value->id . ")' style='background-color: {$label}; color: #000000;'><i class='fa fa-check'></i> </a>";
                }
                ...
                // if($status=='Approved'){
                //     $appointmentid = $this->customlib->getSessionPrefixByType('appointment') . $value->id;
                // }else{
                //     $appointmentid = '';
                // }
                $appointmentid = "APPN" . $this->customlib->getSessionPrefixByType('appointment') . $value->id;
                //==============================
                $row[] = $first_action . composePatientName($value->patient_name, $value->pid) . "</a>" . $action;
                $row[] = $appointmentid;
                $row[] = $value->appointment_serial_no;
                $row[] = date('m/d/Y', strtotime($value->date));
                $row[] = $value->mobileno;
                $row[] = $value->gender;
                $row[] = composeStaffNameByString($value->name, $value->surname, $value->employee_id);
                $row[] = date("h:i A", strtotime($value->start_time)) . '-' . date("h:i A", strtotime($value->end_time));
    ...
    public function lastopdid()
    {
        ini_set('display_errors', 1);
        error_reporting(E_ALL);

        $patientid = $this->input->get('patient_id', true);
        $type = $this->input->get('type', true);
        $patientid = (int)$patientid;

        if ($type == "ipd") {
            $sql = "SELECT ipd_details.id, ipd_details.case_reference_id, patient_charges.ipd_id
                    FROM ipd_details
                    JOIN patient_charges ON ipd_details.id = patient_charges.ipd_id
                    WHERE ipd_details.id = ?
                    ORDER BY ipd_details.id DESC
                    LIMIT 1";
        } else {
            $sql = "SELECT opd_details.id, opd_details.case_reference_id, patient_charges.opd_id
                    FROM opd_details
                    JOIN patient_charges ON opd_details.id = patient_charges.opd_id
                    WHERE opd_details.patient_id = ?
                    ORDER BY opd_details.id DESC
                    LIMIT 1";
        }
        $query = $this->db->query($sql, $patientid);
        if ($query->num_rows() > 0) {
            $patientlist = $query->row_array();
    ...
    public function get_patientid_by_aayush_unique_id()
    {
        $aayush_unique_id = $this->input->get('aayush_unique_id', true);
        $patientid = "SELECT id AS Patient_id FROM patients WHERE aayush_unique_id=?";
        $query = $this->db->query($patientid, $aayush_unique_id);
        if ($query->num_rows() > 0) {
    ...
    public function getipdbillinglist()
    {
        $userdata = $this->session->userdata('hospitaladmin');
        $accessToken = $userdata['accessToken'] ?? '';
        $api_base_url = $this->config->item('api_base_url');
        $ipdid = isset($_GET['ipdid']) ? intval($_GET['ipdid']) : 0;
        $draw = isset($_GET['draw']) ? intval($_GET['draw']) : 1;
        $limit = isset($_GET['limit']) ? intval($_GET['limit']) : 10;
        $page = isset($_GET['page']) ? intval($_GET['page']) : 0;
        $search = isset($_GET['search']) ? $_GET['search'] : '';
        $url = $api_base_url . "billing/v2/get_billing_details_by_ipdID?ipd_id=$ipdid&search=$search&limit=$limit&page=$page";
        $ch = curl_init();
        curl_setopt($ch, CURLOPT_URL, $url);
        curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
        curl_setopt($ch, CURLOPT_HTTPHEADER, [
    ...
    public function getopdbillinglist()
    {
        $userdata = $this->session->userdata('hospitaladmin');
        $accessToken = $userdata['accessToken'] ?? '';
        $api_base_url = $this->config->item('api_base_url');
        $opd_id = isset($_GET['opd_id']) ? intval($_GET['opd_id']) : 0;
        $draw = isset($_GET['draw']) ? intval($_GET['draw']) : 1;
        $limit = isset($_GET['limit']) ? intval($_GET['limit']) : 10;
        $page = isset($_GET['page']) ? intval($_GET['page']) : 0;
        $search = isset($_GET['search']) ? $_GET['search'] : '';

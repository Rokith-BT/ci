...
class Visitors extends Admin_Controller
{
    ...
    public function __construct()
    {
        ...
        if (!$this->Admin_model->validationModule('front_office')) {
            access_denied();
        }
        ...
        if (!empty($dt_response->data)) {
            foreach ($dt_response->data as $key => $value) {
                ...
                if ($value->image !== "") {
                    ...
                    $action .= "<a href='data:" . $mimeType . ";base64," . $base64File . "' download='" . htmlspecialchars($fileInfo['basename']) . "' class='btn btn-default btn-xs' data-toggle='tooltip' title='" . $this->lang->line('download') . "'><i class='fa fa-download'></i></a>";
                }
                ...
                if ($this->rbac->hasPrivilege('visitor_book', 'can_delete')) {
                    ...
                    // if ($value->image !== "") {
                    //     $action .= "<a href='#' onclick=deletevisitorimage('" . $value->id ."') class='btn btn-default btn-xs pull-right'  data-toggle='tooltip' title=''  data-original-title=" . $this->lang->line('delete') . "><i class='fa fa-trash' aria-hidden='true'></i></a>";
                    // } else {

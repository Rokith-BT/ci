...
class patient extends Admin_Controller
{
    ...
    public function getCharge()
    {
        ...
        $appoitmentcharges   = $this->charge_model->getAppointmentCharges($id);
        $result->charge_date = $this->customlib->YYYYMMDDHisTodateFormat($result->date, $this->customlib->getHospitalTimeFormat());
        $array               = array('status' => 1, 'result' => $result, 'appoimentcharges' => $appoitmentcharges, 'message' => $this->lang->line('success_message'));
        echo json_encode($array);
    }
 ...
 */
    ...
    {
        log_message('error', 'bulk_delete method started.');
        ...
        $this->form_validation->set_rules(
            ...
            array('required' => $this->lang->line('no_record_selected'))
        );
        ...
        if ($this->form_validation->run() == false) {
            ...
            log_message('error', 'bulk_delete method validation failed. Errors: ' . json_encode($msg));
        } else {
            try {
                $patient = $this->input->post('delete_id');
                log_message('error', 'Deleting patient records with IDs: ' . json_encode($patient));
                $this->patient_model->bulkdelete($patient);
                $return_array = array('status' => 1, 'error' => '', 'msg' => $this->lang->line('delete_message'));
                log_message('error', 'bulk_delete method completed successfully.');
            } catch (Exception $e) {
                log_message('error', 'Exception in bulk_delete method: ' . $e->getMessage());
                $return_array = array('status' => 0, 'error' => $e->getMessage(), 'msg' => 'An error occurred during deletion.');
            }
        }
    ...
    public function addpatient()
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            $patient_data = array(
                ...
                'ABHA_number' => $this->input->post('identification_number'),
                'is_active'             => 'yes',
            );
 ...
 */
    public function import()
    {
        ...
        $fields         = array('patient_name', 'guardian_name', 'gender', 'age', 'month', 'day', 'marital_status', 'mobileno', 'email', 'address', 'note', 'known_allergies', 'ABHA_number', 'insurance_id', 'insurance_validity');
        $data["fields"] = $fields;
    ...
    public function search()
    {
        ...
        if (!$this->Admin_model->validationModule('opd')) {
            access_denied();
        }
        ...
        $doctors                    = $this->staff_model->opdstaff(3);
        ...
        $url = $api_base_url . 'opd-out-patient/v2/getAllpage/opd?limit=' . $limit . '&page=' . $page . '&search=' . $search;
    ...
    public function getopddatatable()
    {
        ...
        if (!empty($dt_response->data)) {
            foreach ($dt_response->data as $key => $value) {
                ...
                $encoded_pid = !empty($value->pid) ? base64_encode($value->pid) : '-';

                $action .= "<a href='" . base_url() . "admin/patient/profile/" . $encoded_pid . "' class='btn btn-default btn-xs' data-toggle='tooltip' title='" . $this->lang->line('show') . "'><i class='fa fa-reorder' aria-hidden='true'></i></a>";
                $action .= "</div>";

                $first_action = "<a href='" . base_url() . "admin/patient/profile/" . $encoded_pid . "'>";

                $row[] = $first_action . ($value->patient_name ?? '-') . "</a>" . $action;
                $row[] = $value->patientid ?? '-';
                $row[] = "<div class='text-center'>" . ($value->guardian_name ?? '-') . "</div>";
                $row[] = "<div class='text-center'>" . ($value->gender ?? '-') . "</div>";
                $row[] = "<div class='text-center'>" . ($value->mobileno ?? '-') . "</div>";
                $row[] = "<div class='text-center'>" . (!empty($value->name) ? composeStaffNameByString($value->name ?? '-', $value->surname ?? '-', $value->employee_id ?? '-') : '-') . "</div>";
                $row[] = !empty($value->last_visit) ? date('d/m/Y', strtotime($value->last_visit)) : '-';
    ...
    public function getopdvisitdatatable()
    {
        ...
        if (!empty($dt_response->data)) {
            foreach ($dt_response->data as $key => $value) {
                ...
                $action .= "</div>";
                $first_action = "<a href='" . base_url() . "admin/patient/visitdetails/" . base64_encode($value->pid) . "/" . base64_encode($opd_id) . "'>";
                $appointment_action = "<a href='javascript:void(0);' onclick='viewDetail(" . $value->appointment_id . ")'>APPN" . $value->appointment_id . "</a>";
                //==============================
                $row[] = "<div class='text-center'>" . $first_action . ($this->opd_prefix . ($opd_id ?? '-')) . "</a></div>";
                $row[] = "<div class='text-center'>" . ($appointment_action ?? '-') . "</div>";
                $row[] = "<div class='text-center'>" . ($value->case_reference_id ?? '-') . "</div>";
                $row[] = "<div class='text-center'>" . (!empty($value->appointment_date) ? date('d/m/Y', strtotime($value->appointment_date)) : '-') . "</div>";
                $row[] = "<div class='text-center'>" . (!empty($value->name) ? composeStaffNameByString($value->name, $value->surname, $value->employee_id) : '-') . "</div>";
                $row[] = "<div class='text-center'>" . (!empty(trim($value->refference)) ? trim($value->refference) : '-') . "</div>";
                $row[] = "<div class='text-center'>" . (!empty($value->symptoms) ? nl2br($value->symptoms) : '-') . "</div>";
                //====================
                if (!empty($fields)) {
                    foreach ($fields as $fields_key => $fields_value) {
                        $display_field = $value->{"$fields_value->name"};
    ...
    public function getvisitdatatable($opdid)
    {
        ...
        if (!empty($dt_response->data)) {
            foreach ($dt_response->data as $key => $value) {
                ...
                $check = $this->db->where("visit_details_id", $visit_details_id)->get('ipd_prescription_basic');
                $result[$key]['prescription'] = ($check->num_rows() > 0) ? 'yes' : 'no';

                if ($result[$key]['prescription'] === 'no') {
                    $userdata = $this->customlib->getUserData();
                    if ($this->session->has_userdata('hospitaladmin')) {
                        $doctor_restriction = $this->session->userdata['hospitaladmin']['doctor_restriction'] ?? '';
                        if ($doctor_restriction == 'enabled' && $userdata["role_id"] == 3 && $userdata["id"] != $value->staff_id) {
                            $result[$key]['prescription'] = 'not_applicable';
                        }
                    }
                }
                ...
                $row[] = "<div class='text-center'>" . $this->customlib->getSessionPrefixByType('checkup_id') . ($visit_details_id ?? '-') . "</div>";
                $row[] = "<div class='text-center'>" . (!empty($value->appointment_date) ? date('d/m/Y', strtotime($value->appointment_date)) : '-') . "</div>";
                $row[] = "<div class='text-center'>" . (!empty($value->name) ? composeStaffNameByString($value->name, $value->surname, $value->employee_id) : '-') . "</div>";
                $row[] = "<div class='text-center'>" . (!empty(trim($value->refference)) ? trim($value->refference) : '-') . "</div>";
                $row[] = "<div class='text-center'>" . (!empty($value->symptoms) ? nl2br($value->symptoms) : '-') . "</div>";
                ...
                if (!empty($fields)) {
                    foreach ($fields as $fields_key => $fields_value) {
                        $display_field = $value->{"$fields_value->name"} ?? '-';
                        if ($fields_value->type == "link" && !empty($value->{"$fields_value->name"})) {
                            $display_field = "<a href='{$value->{$fields_value->name}}' target='_blank'>{$value->{$fields_value->name}}</a>";
                        }
                        $row[] = "<div class='text-center'>{$display_field}</div>";
                    }
                }
    ...
    public function ipdsearch($bedid = '', $bedgroupid = '')
    {
        ...
        if (!$this->Admin_model->validationModule('ipd')) {
            access_denied();
        }
        ...
        $page = isset($_GET['page']) ? intval($_GET['page']) : 1;
        $search = isset($_GET['search']) ? $_GET['search'] : '';
        $url = $api_base_url . 'ipd-main-module/v2/ipd_details?limit=' . $limit . '&page=' . $page . '&search=' . $search;
    ...
    public function getipddatatable()
    {
        ...
        if (!empty($dt_response->data)) {
            foreach ($dt_response->data as $key => $value) {
                ...
                $appointment_action = '<a href="javascript:void(0);" onclick="viewDetail(' . ($value->appointment_id ?? '-') . ')">';

                $row[] = $first_action . $this->customlib->getSessionPrefixByType('ipd_no') . ($value->ipdid ?? '-') . "</a>" . $action;
                $row[] = $appointment_action . $this->customlib->getSessionPrefixByType('appointment') . ($value->appointment_id ?? '-') . "</a>" . $action;
                $row[] = !empty($value->case_reference_id) ? $value->case_reference_id : '-';
                $row[] = !empty($value->patient_name) ? composePatientName($value->patient_name, $value->id) : '-';
                $row[] = !empty($value->gender) ? $value->gender : '-';
                $row[] = !empty($value->mobileno) ? $value->mobileno : '-';
                $row[] = (!empty($value->name) || !empty($value->surname) || !empty($value->employee_id))
                    ? composeStaffNameByString($value->name, $value->surname, $value->employee_id)
                    : '-';
                $row[] = (!empty($value->bed_name) || !empty($value->bedgroup_name) || !empty($value->floor_name))
                    ? $value->bed_name . "-" . $value->bedgroup_name . "-" . $value->floor_name
                    : '-';
                ...
                $row[] = !empty($value->ipdcredit_limit) ? amountFormat($value->ipdcredit_limit) : '-';
    ...
    public function discharged_patients()
    {
        ...
        $url = $api_base_url . 'discharge-patient-ipd-module/v2/ipd_discharge_patient?limit=' . $limit . '&page=' . $page . '&search=' . urlencode($search);
        ...
        curl_close($ch);
        $responseData = json_decode($response, true);
        $data["ipd_discharge_patient_list"] = [
            "draw" => $draw,
            "recordsTotal" => isset($responseData['total']) ? (int)$responseData['total'] : 0,
            "recordsFiltered" => isset($responseData['total']) ? (int)$responseData['total'] : 0,
            "data" => isset($responseData['data']) ? $responseData['data'] : []
        ...
        $this->session->set_userdata('top_menu', 'IPD_in_patient');
        $this->load->view('layout/header');
        $this->load->view('admin/patient/dischargedPatients', $data);
        $this->load->view('layout/footer');
    }
    ...
    {
        $id = base64_decode($id);
        $opdid = base64_decode($opdid);
        if (!$this->Admin_model->validationModule('opd')) {
            access_denied();
        }
        if (!empty($id)) {
            $result         = $this->patient_model->getDetails($opdid);
            $first_action = "<a href='" . base_url('admin/appointment/index/' . $result["appointment_id"]) . "'>";
            ...
            $symptoms_resulttype        = $this->symptoms_model->getsymtype();
            $data['symptomsresulttype'] = $symptoms_resulttype;
            $doctors                    = $this->staff_model->getStaffbyrole(3, 'opd');
            $data["doctors"]            = $doctors;
            ...
            $timeline_list         = $this->timeline_model->getPatientTimeline($data["id"], $timeline_status = '');
            ...
            $data['prescription_detail']  = $this->patient_model->getprescription_detail($opdid);
            ...
            $url = $api_base_url . "internal-opd-overview-visits/v2/getAllpagess?patient_id={$result['pid']}&opd_details_id={$opdid}&limit={$limit}&page={$page}&search={$search}";
            $urlcharged = $api_base_url . "internal-opd-charges/v2/getAllcharges?patient_id={$result['pid']}&opdDetailsId={$opdid}&limit={$limit}&page={$page}&search={$search}";
            $urlpayment = $api_base_url . "internal-opd-payment/v2/getAllopdpayment?patient_id={$result['pid']}&opdDetailsId={$opdid}&limit={$limit}&page={$page}&search={$search}";

            $makeRequest = function ($url, $accessToken) {
                $ch = curl_init();
                curl_setopt($ch, CURLOPT_URL, $url);
                curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
                curl_setopt($ch, CURLOPT_HTTPHEADER, [
            ...
            $responseData = $makeRequest($url, $token);
            $data["ocdlist"] = [
                "draw" => $draw,
                "recordsTotal" => isset($responseData['total']) ? (int)$responseData['total'] : 0,
                "recordsFiltered" => isset($responseData['total']) ? (int)$responseData['total'] : 0,
            ...
            $responseDataCharges = $makeRequest($urlcharged, $token);
            $data["chargeslist"] = [
                "draw" => $draw,
                "recordsTotal" => isset($responseDataCharges['total']) ? (int)$responseDataCharges['total'] : 0,
                "recordsFiltered" => isset($responseDataCharges['total']) ? (int)$responseDataCharges['total'] : 0,
                "data" => isset($responseDataCharges['data']) ? $responseDataCharges['data'] : []
            ];

            $responseDataPayments = $makeRequest($urlpayment, $token);
            $data["paymentlist"] = [
                "draw" => $draw,
                "recordsTotal" => isset($responseDataPayments['total']) ? (int)$responseDataPayments['total'] : 0,
                "recordsFiltered" => isset($responseDataPayments['total']) ? (int)$responseDataPayments['total'] : 0,
                "data" => isset($responseDataPayments['data']) ? $responseDataPayments['data'] : []
            ];
    ...
    public function profile($id)
    {
        ...
        $data['patient_id'] = $id;
        $api_base_url = $this->config->item('api_base_url');
        $patientid = isset($_GET['patientid']) ? intval($_GET['patientid']) : $data['patient_id'];
        $draw = isset($_GET['draw']) ? intval($_GET['draw']) : 1;
        $limit = isset($_GET['limit']) ? intval($_GET['limit']) : 10;
        $page = isset($_GET['page']) ? intval($_GET['page']) : 1;
        $search = isset($_GET['search']) ? $_GET['search'] : '';
        $hospitaldata = $this->session->userdata('hospitaladmin');
        $token = $hospitaldata['accessToken'] ?? '';
        $url = $api_base_url . 'internal-opd-overview-visits/v2/getAllpage?patient_id=' . $patientid . '&limit=' . $limit . '&page=' . $page . '&search=' . $search;
        $makeRequest = function ($accessToken) use ($url) {
            $ch = curl_init();
            curl_setopt($ch, CURLOPT_URL, $url);
            curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
    ...
    public function ipdprofile($ipdid)
    {
        ...
        $urlCharges = $api_base_url . 'internal-ipd-charges/v2/get_ipd_charges?' . $queryParams;
        $urlPayments = $api_base_url . 'internal-ipd-payment/v2/get_ipd_payments?' . $queryParams;
        $urlDoctors = $api_base_url . 'consultant-register-ipd/v2/get_consultant_register_ipd_details?' . $queryParams;
        $userdata = $this->session->userdata('hospitaladmin');
        $accessToken = $userdata['accessToken'] ?? '';

        $chCharges = curl_init();
        curl_setopt($chCharges, CURLOPT_URL, $urlCharges);
        curl_setopt($chCharges, CURLOPT_RETURNTRANSFER, true);
        curl_setopt($chCharges, CURLOPT_HTTPHEADER, [
            'Content-Type: application/json',
            'Authorization: ' . $accessToken
        ]);

        $responseCharges = curl_exec($chCharges);
        curl_close($chCharges);

        $chPayments = curl_init();
        curl_setopt($chPayments, CURLOPT_URL, $urlPayments);
        ...
        $responsePayments = curl_exec($chPayments);
        curl_close($chPayments);

        $chDoctors = curl_init();
        curl_setopt($chDoctors, CURLOPT_URL, $urlDoctors);
        curl_setopt($chDoctors, CURLOPT_RETURNTRANSFER, true);
        curl_setopt($chDoctors, CURLOPT_HTTPHEADER, [
            'Content-Type: application/json',
            'Authorization: ' . $accessToken
        ]);
        $responseDoctor = curl_exec($chDoctors);
        curl_close($chDoctors);

        $responseDataCharges = json_decode($responseCharges, true);
        $responseDataPayments = json_decode($responsePayments, true);
        $responseDataDoctors = json_decode($responseDoctor, true);

        $data["ipdcharges"] = [
            "draw" => $draw,
            "recordsTotal" => isset($responseDataCharges['total']) ? (int)$responseDataCharges['total'] : 0,
            "recordsFiltered" => isset($responseDataCharges['total']) ? (int)$responseDataCharges['total'] : 0,
            "data" => $responseDataCharges['data'] ?? []
        ];
        ...
        $data["paymentdoneapi"] = [
            ...
            "recordsTotal" => isset($responseDataPayments['total']) ? (int)$responseDataPayments['total'] : 0,
            "recordsFiltered" => isset($responseDataPayments['total']) ? (int)$responseDataPayments['total'] : 0,
            "data" => $responseDataPayments['data'] ?? []
        ];

        $data["doctordetails"] = [
            "draw" => $draw,
            "recordsTotal" => isset($responseDataDoctors['total']) ? (int)$responseDataDoctors['total'] : 0,
            "recordsFiltered" => isset($responseDataDoctors['total']) ? (int)$responseDataDoctors['total'] : 0,
            "data" => $responseDataDoctors['data'] ?? []
        ];
    ...
    public function getopdDetails()
    {
        ...
        $opd_id = $this->input->post('opd_id');
        $api_base_url = $this->config->item('api_base_url');
        $url = $api_base_url . 'internal-opd-overview-visits/' . $opd_id;
        $userdata = $this->session->userdata('hospitaladmin');
        $accessToken = $userdata['accessToken'] ?? '';
        ...
        try {
            ...
            if (is_array($responseData) && isset($responseData[0])) {
                ...
                echo json_encode([
                    'status' => 1,
                    'page' => $page,
                    'visitid' => isset($responseData[0]["OPD_checkup_id"]) ? (int)preg_replace('/\D/', '', $responseData[0]["OPD_checkup_id"]) : null
                ]);
            } else {
                echo json_encode(['status' => 0, 'message' => 'Invalid or empty response']);
            }
        } catch (Exception $e) {
    ...
    public function getopdrecheckupDetails()
    {
        ...
        $api_base_url = $this->config->item('api_base_url');
        $visitid        = $this->input->post("visit_id");
        $url = $api_base_url . 'internal-opd-overview-visits/ocid/' . $visitid;
        $userdata = $this->session->userdata('hospitaladmin');
        $accessToken = $userdata['accessToken'] ?? '';
        ...
        $data["prefix"] = [
            "opdprefix"=>$this->customlib->getSessionPrefixByType('opd_no'),
            "Recheckupprefix"=>$this->customlib->getSessionPrefixByType('checkup_id')
        ];
        ...
        echo json_encode(array('status' => 1, 'page' => $page, 'can_delete' => "no"));
    }
    ...
    public function getpatientDetails()
    {
        ...
        if (($result['insurance_validity'] == '') || ($result['insurance_validity'] == '0000-00-00') || ($result['insurance_validity'] == '1970-01-01')) {
            ...
            $result['insurance_validity'] = date("d/m/Y", strtotime($result['insurance_validity']));
        }
    ...
    public function printbill()
    {
        ...
        if (!empty($opddata)) {
            ...
            if (isset($type) && $type == "all") {
                $charge            = $this->charge_model->getChargeByall($opd_id);
            } else {
    ...
    public function printCharge()
    {
        ...
        $data['print_details'] = $print_details;
        $data['charge']        = $charge;
        $page                  = $this->load->view('admin/patient/ipdchargespdf', $data, true);
        echo json_encode(array('status' => 1, 'page' => $page));
    }
        ...
        if (!empty($search_term)) {
            ...
            if (!empty($result)) {
                foreach ($result as $value) {
                    $data[] = array(
                        "id"   => $value->id,
                        "text" => $value->patient_name . " ( ID: " . $value->id . ", Mobile: " . $value->mobileno . ")"
                    );
                }
            }
            echo json_encode($data);
    ...
    public function getpatientage()
    {
        ...
        $dob = DateTime::createFromFormat('m/d/Y', $birth_date);
        $agr_array = array();
        ...
        if ($dob !== false) {
            ...
            $age = $diff->y;
            if ($diff->m >= 6 || ($diff->m == 5 && $diff->d >= 15)) {
                $age += 1;
            }
    ...
    public function getopdtreatmenthistory()
    {
        ...
        if (!empty($dt_response->data)) {
            foreach ($dt_response->data as $key => $value) {
                ...
                $action .= "<a href='javascript:void(0)' data-loading-text='" . $this->lang->line('please_wait') . "' data-opd-id=" . $opd_id . " data-record-id=" . $visit_details_id . " class='btn btn-default btn-xs get_opd_detail'  data-toggle='tooltip' title='" . $this->lang->line('show') . "'><i class='fa fa-reorder'></i></a>";
                $action .= "</div>";
                $encoded_pid = base64_encode($value->pid);
                $encoded_opd_id = base64_encode($opd_id);
                $first_action = '<a href="' . base_url() . 'admin/patient/visitdetails/' . $encoded_pid . '/' . $encoded_opd_id . '">';
    ...
    public function getopdpaymentdetails()
    {
        ...
        $result['edit_payment_id']   = $payment_id;
        echo json_encode($result);
    }
    ...
    public function getcasesheet()
    {
        $opdid = $_GET['opdid'];
        $sql = "SELECT case_sheet_document FROM visit_details WHERE opd_details_id = ?";
        $result = $this->db->query($sql, [$opdid])->row_array();
        echo json_encode($result);
    }

    public function getopdpatientlist()
    {
        $api_base_url = $this->config->item('api_base_url');
        $draw = isset($_GET['draw']) ? intval($_GET['draw']) : 1;
        $limit = isset($_GET['limit']) ? intval($_GET['limit']) : 10;
        ...
        $url = $api_base_url . 'opd-out-patient/v2/getAllpage/opd?limit=' . $limit . '&page=' . $page . '&search=' . urlencode($search);
    ...
    public function getopdvisitdetials()
    {
        $api_base_url = $this->config->item('api_base_url');
        $patientid = isset($_GET['patientid']) ? intval($_GET['patientid']) : 1;
        $draw = isset($_GET['draw']) ? intval($_GET['draw']) : 1;
        $limit = isset($_GET['limit']) ? intval($_GET['limit']) : 10;
        $page = isset($_GET['page']) ? intval($_GET['page']) : 0;
        $search = isset($_GET['search']) ? $_GET['search'] : '';
    ...
    public function getocddetials()
    {
        $api_base_url = $this->config->item('api_base_url');
        $patient_id = isset($_GET['patientid']) ? intval($_GET['patientid']) : 1;
        $opd_details_id = isset($_GET['opd_details_id']) ? intval($_GET['opd_details_id']) : 0;
        $draw = isset($_GET['draw']) ? intval($_GET['draw']) : 1;
        $limit = isset($_GET['limit']) ? intval($_GET['limit']) : 10;
        $page = isset($_GET['page']) ? intval($_GET['page']) : 1;
        $search = isset($_GET['search']) ? urlencode($_GET['search']) : '';
        $hospitaldata = $this->session->userdata('hospitaladmin');
        $token = $hospitaldata['accessToken'] ?? '';
        $url = $api_base_url . "internal-opd-overview-visits/v2/getAllpagess?patient_id={$patient_id}&opd_details_id={$opd_details_id}&limit={$limit}&page={$page}&search={$search}";
        $makeRequest = function ($accessToken) use ($url) {
            $ch = curl_init();
            curl_setopt($ch, CURLOPT_URL, $url);
            curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
    ...
    public function getipdpatientlist()
    {
        $userdata = $this->session->userdata('hospitaladmin');
        $accessToken = $userdata['accessToken'] ?? '';
        $api_base_url = $this->config->item('api_base_url');
    ...
    public function ipddischargedPatients()
    {
        error_reporting(E_ALL);
        ini_set('display_errors', 1);
        $userdata = $this->session->userdata('hospitaladmin');
        ...
        $page = isset($_GET['page']) ? intval($_GET['page']) : 0;
        $search = isset($_GET['search']) ? $_GET['search'] : '';
        $url = $api_base_url . 'discharge-patient-ipd-module/v2/ipd_discharge_patient?limit=' . $limit . '&page=' . $page . '&search=' . $search;
        $ch = curl_init();
        curl_setopt($ch, CURLOPT_URL, $url);
        curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
        curl_setopt($ch, CURLOPT_HTTPHEADER, [
    ...
    public function ipdchargedlist()
    {
        $userdata = $this->session->userdata('hospitaladmin');
        $accessToken = $userdata['accessToken'] ?? '';
        $api_base_url = $this->config->item('api_base_url');
        ...
        $page = isset($_GET['page']) ? intval($_GET['page']) : 1;
        $search = isset($_GET['search']) ? $_GET['search'] : '';
        $ipdid = isset($_GET['ipd_id']) ? $_GET['ipd_id'] : '';
        ...
        $url = $api_base_url . 'internal-ipd-charges/v2/get_ipd_charges?' . $queryParams;
        $ch = curl_init();
        curl_setopt($ch, CURLOPT_URL, $url);
        curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
        curl_setopt($ch, CURLOPT_HTTPHEADER, [
    ...
    public function ipdpaymentlist()
    {
        $userdata = $this->session->userdata('hospitaladmin');
        $accessToken = $userdata['accessToken'] ?? '';
        $api_base_url = $this->config->item('api_base_url');
        ...
        $urlPayments = $api_base_url . 'internal-ipd-payment/v2/get_ipd_payments?' . $queryParams;
        $ch = curl_init();
        curl_setopt($ch, CURLOPT_URL, $urlPayments);
        curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
        curl_setopt($ch, CURLOPT_HTTPHEADER, [
            'Content-Type: application/json',
            'Authorization: ' . $accessToken
    ...
    public function ipddoctorslist()
    {
        $userdata = $this->session->userdata('hospitaladmin');
        $accessToken = $userdata['accessToken'] ?? '';
        $api_base_url = $this->config->item('api_base_url');
    ...
    public function getocdcharges()
    {
        ...
        $page = isset($_GET['page']) ? intval($_GET['page']) : 1;
        $search = isset($_GET['search']) ? urlencode($_GET['search']) : '';
        $opdid = isset($_GET['opd_details_id']) ? $_GET['opd_details_id'] : '';
        $patientid = isset($_GET['patientid']) ? $_GET['patientid'] : '';

        $hospitaldata = $this->session->userdata('hospitaladmin');
        $token = $hospitaldata['accessToken'] ?? '';

        $urlcharged = $api_base_url . "internal-opd-charges/v2/getAllcharges?patient_id={$patientid}&opdDetailsId={$opdid}&limit={$limit}&page={$page}&search={$search}";

        $ch = curl_init();
        curl_setopt($ch, CURLOPT_URL, $urlcharged);
        curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
        curl_setopt($ch, CURLOPT_HTTPHEADER, [
            'Content-Type: application/json',
            'Authorization: ' . $token
    ...
    public function getocdpayment()
    {
        ...
        $urlpayment = $api_base_url . "internal-opd-payment/v2/getAllopdpayment?patient_id={$patientid}&opdDetailsId={$opdid}&limit={$limit}&page={$page}&search={$search}";
        ...
        $responseDataPayment = json_decode($responsePayment, true);

        $data = [
            "draw" => $draw,
            "recordsTotal" => isset($responseDataPayment['total']) ? (int)$responseDataPayment['total'] : 0,
            "recordsFiltered" => isset($responseDataPayment['total']) ? (int)$responseDataPayment['total'] : 0,
            "data" => isset($responseDataPayment['data']) ? $responseDataPayment['data'] : []
        ];

        echo json_encode($data);
    }

    public function getopdreport()
    {
        $search_type = $this->input->post('search_type');
        $doctor_id = $this->input->post('doctor');
        $fromAge = $this->input->post('from_age');
        $toAge = $this->input->post('to_age');
        $gender = $this->input->post('gender');
        $symptoms = $this->input->post('symptoms');
        $findings = $this->input->post('findings');
        $date_from = $this->input->post('date_from');
        ...
        $hospitaldata = $this->session->userdata('hospitaladmin');
        $token = $hospitaldata['accessToken'] ?? '';
        $url = $api_base_url . 'php-opd-report?page=' . $page . '&search=' . $search_value . '&limit=' . $limit;
    ...
    public function getipdreport()
    {
        ...
        $hospitaldata = $this->session->userdata('hospitaladmin');
        $token = $hospitaldata['accessToken'] ?? '';
        $url = $api_base_url . 'php-ipd-report?page=' . $page . '&search=' . $search_value . '&limit=' . $limit;
        $makeRequest = function ($accessToken) use ($url, $data) {
            $ch = curl_init();
            curl_setopt($ch, CURLOPT_URL, $url);
            curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);

...
class Pharmacy extends Admin_Controller
{
    ...
    public function __construct()
    {
        ...
        if (!$this->Admin_model->validationModule('pharmacy')) {
            access_denied();
        }
    }
    ...
    public function getpharmacyDatatable()
    {
        ...
        if (!empty($dt_response->data)) {
            foreach ($dt_response->data as $key => $value) {
                ...
                $available_qty = ($value->total_qty - $used_quantity)-$value->total_medicine_bad_stock;
                $action .= "<div'>";
...
//======================================================================================
        ...
        if (!empty($dt_response->data)) {
            foreach ($dt_response->data as $key => $value) {
                ...
                if (!empty($value->file)) {
                    ...
                    $base64File = trim($response);  // Ensure no extra whitespace
                    $fileData = base64_decode($base64File);
    
                    if ($fileData === false) {
                        echo "Failed to decode base64 file data.";
                        return;
                    }
    
                    // Extract base name and file extension
                    $fileInfo = pathinfo($name);
                    $baseName = isset($fileInfo['filename']) ? $fileInfo['filename'] : 'medicalimage';
                    $fileExtension = isset($fileInfo['extension']) ? $fileInfo['extension'] : 'bin';
                    ...
                    // Construct a fixed filename with the correct extension
                    $fixedFileName = 'medicalimage.' . $fileExtension;
    
                    // Construct data URL and action link
                    $action .= "<a href='data:" . $mimeType . ";base64," . $base64File . "' download='" . $fixedFileName . "' class='btn btn-default btn-xs' data-toggle='tooltip' title='" . $this->lang->line('download') . "'><i class='fa fa-download'></i></a>";
                }
    ...
    public function stackcount()
    {
        error_reporting(E_ALL);
        ini_set('display_errors', 1);
        $medicine_batch_id = $_GET['medicine_batch_id'];
        $pharmacy_id = $_GET['pharmacy_id'];
        $result = $this->pharmacy_model->getStackCount($medicine_batch_id, $pharmacy_id);

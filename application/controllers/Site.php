...
class Site extends Public_Controller
{
    ...
    public function login()
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            try {
                ...
                $api_base_url = $this->config->item('api_base_url_casesheet');
                $api_url = $api_base_url . "login";
                $ch = curl_init($api_url);
                curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
                curl_setopt($ch, CURLOPT_POST, true);
                curl_setopt($ch, CURLOPT_POSTFIELDS, json_encode($login_post));
                ...
                if (is_array($result) && isset($result['status']) && $result['status'] === 'success') {
                    ...
                    $session_data = array(
                        ...
                        'time_format' => (($setting_result[0]['time_format'] ?? '') == '12-hour') ? false : true,
                        'prefix' => $this->prefix_model->getPrefixArray() ?? [],
                        'resetStatus' => $details['resetStatus'] ?? '',
                        'accessToken' => $details['accessToken'] ?? ''
                    );
                    $this->session->set_userdata('hospitaladmin', $session_data);
                    $this->customlib->setUserLog($this->input->post('username'), $details['role_name']);
                    if ($session_data['resetStatus'] == '1') {
                        echo json_encode(array('status' => 'reset', 'message' => 'Please reset your password.'));
                    } else {
                    ...
                    throw new Exception($this->lang->line('invalid_username_or_password'));
                }
            } catch (Exception $e) {
    ...
    public function admin_resetpassword($verification_code = null)
    {
        ...
        $this->load->view('admin/admin_resetpassword');
    }
